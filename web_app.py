...
from developer_router import (
    ...
    ai_fallback_response as _developer_ai_fallback_response,
    ollama_model_discovered as _developer_ollama_model_discovered,
)
...
from client_identity import (client_context, get_client_mapping, 
                              get_or_create_client_mapping, list_client_mappings)
...
def _workspace_cookie_secret():
    ...
    if not value:
        value = hashlib.sha256(b'NINA_WORKSPACE_COOKIE_FALLBACK').hexdigest()
    return value.encode()
...
def _admin_developer_body(notice="", ai_response=""):
    ...
    statuses = (
        ...
        ("AI Coding Model", "qwen3.5:27b API available" if ai_ready else "Unavailable"),
    ...
    if ai_response:
        message += "<div class='channel-message' style='white-space:pre-wrap'>" + html_escape(ai_response) + "</div><br>"
    ...
    message_input = (
        ...
        if (developer_ready or ai_ready) else
...
def admin_developer():
    ...
    if request.method == "POST":
        ...
        ai_result = None
        # AI is informational only: no jobs, commands, or release requests are delegated.
        ai_question = bool(command and not investigation_steps and not operation and not intents)
        if ai_question:
            ai_result = _developer_ai_fallback_response(command, context_evidence=None)
            notice = "ai_answer" if ai_result.get("success") else "ai_error"
        elif not _developer_connection_ready(status):
        ...
        if request.accept_mimetypes.best == "application/json":
            messages = {
                ...
                "ai_answer": "Lokālā AI atbilde (bez repozitorija pierādījumiem):",
                "ai_error": "Lokālais AI nav pieejams.",
            }
            if ai_result is not None:
                return jsonify({"ok": bool(ai_result.get("success")), "notice": notice,
                                "message": (ai_result.get("text") if ai_result.get("success")
                                            else ai_result.get("error", "AI kļūda.")),
                                "model": ai_result.get("model", "qwen3.5:27b")}), (200 if ai_result.get("success") else 503)
            return jsonify({"ok": notice == "job_created", "notice": notice,
                            "message": messages.get(notice, "")}), (200 if notice == "job_created" else 409)
    return Response(
        page("Nina Developer", _admin_developer_body(notice,
             (ai_result.get("text") if ai_result.get("success") else ai_result.get("error", "AI kļūda."))
             if request.method == "POST" and ai_result is not None else ""), active="admin"),
        mimetype="text/html",
    )
...
def channels_company_whatsapp_connect():
    ...
    try:
        ...
        logger.exception("Company WhatsApp pairing could not start")
        try:
...
def internal_personal_whatsapp_inbound():
    print("PERSONAL_WA_INBOUND_V1", flush=True)
    payload = _bridge_json()
    if payload is None: return jsonify({"ok": False}), 401
    workspace_id = str(payload.get("workspace_id") or "")
    ...
    if media:
        try:
            ...
            result = process_media_message(
                ...
                channel=PERSONAL_WHATSAPP_CHANNEL,
            ...
            logger.exception(
                "Personal WhatsApp media processing failed: error_class=%s",
                type(exc).__name__,
            )
            return jsonify({
                "ok": True,
                "accepted": True,
                "reply": f"MEDIA_ERROR:{type(exc).__name__}:{str(exc)}"
            })
    else:
    ...
    if request.method == "POST":
        ...
        if user_text:
            ...
            if nina_result is None:
                canonical_client_id = ""
                mapping = get_client_mapping(NINA_WEB_WORKSPACE_ID, contact["contact_id"])
                if mapping:
                    canonical_client_id = str(mapping.get("canonical_client_id") or "")
