...
try:
    from work_objects import (
        ...
        canonical_action_result as one_nina_canonical_action_result,
    ...
    def one_nina_canonical_action_result(*args, **kwargs):
        return {}
...
try:
    ...
    print("V51.7 ONE NINA estimate action import error:", repr(e))
...
WEB_APP_VERSION = "Web App V51.7 — ONE NINA Canonical Estimate Action V1"
app = Flask(__name__)
...
def one_nina_estimate_action_html(obj):
    """Render the persisted estimate action result from this same Work Object."""
    ...
    if draft_text:
        return (
            ...
            "<div class='safe-note'>Saved on this same canonical estimate Work Object.</div>"
            "</div>"
        )
...
def prepare_estimate_draft(object_id):
    """Web is only an action surface; Work Engine performs the ONE NINA action."""
    ...
    if not result.get("ok"):
        print("V51.7 estimate action error:", result)
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=port) does not classify Telegram text here. It reads the same persistent
# nina_work_objects objects that app.py writes through work_objects.py.
try:
    from work_objects import (
        list_work_objects as one_nina_list_work_objects,
