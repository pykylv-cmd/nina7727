...
def classify_document_work_action(text: str) -> Dict[str, Any]:
    ...
    client_reply_markers = (
        "klients jautā", "klients jauta", "klients prasa", "klients raksta",
        "ko atbildēt klientam", "ko atbildet klientam", "atbildi klientam uz",
        "uztaisi atbildi klientam", "sagatavo atbildi klientam",
        "client asks", "reply to the client", "answer the client question",
        "клиент спрашивает", "что ответить клиенту", "ответь клиенту на",
    )
    if any(x in value for x in client_reply_markers):
        return {"matched": True, "action": "client_reply"}
    ...
    client_markers = (
        ...
        "par so tami klientam", "ko sūtīt klientam", "ko sutit klientam",
        "pārsūtīt klientam", "parsutit klientam", "client message", "message to client",
        "send to client", "сообщение клиенту", "написать клиенту",
    )
...
def _extract_client_document_question(instruction: str) -> str:
    value = _collapse(instruction)
    patterns = (
        r"(?i)klients\s+(?:jautā|jauta|prasa|raksta)\s*[:,-]?\s*(.+?)(?:[,;]\s*(?:uztaisi|sagatavo|uzraksti|atbildi)\s+(?:īsu\s+|isu\s+)?atbildi\s+klientam|$)",
        r"(?i)(?:ko\s+atbildēt\s+klientam|ko\s+atbildet\s+klientam)\s*[:,-]?\s*(.+)$",
        r"(?i)(?:atbildi\s+klientam\s+uz)\s*[:,-]?\s*(.+)$",
    )
...
def _is_estimate_total_question(question: str) -> bool:
    value = _collapse(question).casefold()
    has_total = any(x in value for x in ("kopā", "kopa", "kopēj", "kopej", "gala summa", "pavisam", "total", "summa"))
    has_estimate = any(x in value for x in ("tām", "tam", "estimate", "смет"))
    return has_total and has_estimate


def execute_document_to_client_reply_action(
    *,
    instruction: str,
    source_text: str,
    document_kind: str,
    ...
    """Prepare one forward-ready reply from the SAME canonical document truth."""
    instruction = _clean(instruction)
    source = _clean(source_text)
    facts = list(grounded_facts or [])
    if not instruction or not source or generator is None:
        return {"ok": False, "error": "missing_document_client_reply_source", "answer": ""}

    client_question = _extract_client_document_question(instruction)

    # Deterministic estimate-total path: reuse the exact same grand-total truth
    # already proven by Document-to-Client Action V1.4.
    if _is_estimate_total_question(client_question):
        total = _extract_deterministic_estimate_grand_total(source)
        if not total.get("ok"):
            total = _extract_validated_total_from_followup(
                source_text=source,
                document_kind=document_kind,
                grounded_facts=facts,
                generator=generator,
        ...
        if total.get("ok"):
            evidence = [_clean(total.get("evidence"))] if _clean(total.get("evidence")) else []
            return {
                "ok": True,
                "answer": f"Labdien! Tāmes kopējā summa ir {total['formatted_amount']}. Ja vēlaties, varu precizēt arī konkrētās tāmes pozīcijas.",
                "evidence": evidence[:8],
                "validated_evidence_count": len(evidence[:8]),
                "action": "client_reply",
                "action_version": DOCUMENT_TO_CLIENT_REPLY_VERSION,
                "deliverable_type": "client_reply",
                "owner_forward_ready": True,
                "grounding_mode": str(total.get("grounding_mode") or "canonical_estimate_total"),
                "client_question": client_question,
                "grand_total": total,
            }

    grounded = answer_document_followup(
    ...
    grounded_answer = _clean(grounded.get("answer"))
    evidence = list(grounded.get("evidence") or [])
    if not grounded.get("ok") or not grounded_answer or not evidence:
        return {
            "ok": False,
            "error": "no_validated_document_client_reply",
            "answer": "No šī dokumenta saglabātā teksta nevaru droši sagatavot klientam pamatotu atbildi uz šo jautājumu.",
            "evidence": evidence[:8],
        }

    answer = grounded_answer
    if not answer.casefold().startswith(("labdien", "sveiki", "čau", "chau")):
        answer = "Labdien! " + answer[:1].upper() + answer[1:]
    if answer and answer[-1] not in ".!?":
        answer += "."
    if "jaut" not in answer.casefold() and "preciz" not in answer.casefold():
        answer += " Ja vēlaties, varu precizēt arī dokumenta detaļas."

    forbidden = ("one nina", "ninaos", "mākslīgais intelekts", "artificial intelligence", " ai ", "versija:")
    lowered = f" {answer.casefold()} "
    if any(marker in lowered for marker in forbidden):
        return {"ok": False, "error": "client_reply_trace_leak", "answer": "", "evidence": evidence[:8]}

    return {
        "ok": True, "answer": answer[:4000], "evidence": evidence[:8],
        "validated_evidence_count": len(evidence[:8]), "action": "client_reply",
        "action_version": DOCUMENT_TO_CLIENT_REPLY_VERSION, "deliverable_type": "client_reply",
        "owner_forward_ready": True, "grounding_mode": "canonical_validated_document_followup_reply",
        "client_question": client_question,
    }
...
def execute_document_work_action(
    *,
    ...
    if action == "client_reply":
        return execute_document_to_client_reply_action(
            instruction=instruction,
            source_text=source,
            document_kind=document_kind,
            grounded_facts=facts,
