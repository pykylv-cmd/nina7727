...
DOCUMENT_WORK_ACTIONS_VERSION = "ONE NINA Document Work Actions V1.3 — Document-to-Client Reply Intelligence V1"
DOCUMENT_TO_CLIENT_ACTION_VERSION = "ONE NINA Document-to-Client Action V1.4 — Production PASS"
DOCUMENT_TO_CLIENT_REPLY_VERSION = "ONE NINA Document-to-Client Reply Intelligence V1"
...
def execute_document_to_client_reply_action(
    *,
    ...
    """Prepare a forward-ready reply to a client's document question from the SAME canonical source.

    The canonical document follow-up engine must first produce an answer backed by exact
    source evidence. This action only turns that validated work answer into owner-forward-ready
    client wording. It does not send anything and does not create a second Work Object.
    """
    instruction = _clean(instruction)
    source = _clean(source_text)
    facts = list(grounded_facts or [])
    ...
    if "jaut" not in answer.casefold() and "preciz" not in answer.casefold():
        answer += " Ja vēlaties, varu precizēt arī konkrētās tāmes pozīcijas."
    ...
    return {
        ...
        "grounding_mode": "canonical_validated_document_followup_reply",
    }
