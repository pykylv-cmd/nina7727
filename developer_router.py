...
# AI fallback constants
OLLAMA_BASE_URL = "http://127.0.0.1:11434"
OLLAMA_MODEL_NAME = "qwen3.5:27b"
AI_FALLBACK_TIMEOUT = 180
...
def ollama_model_discovered():
    """Check if configured model is discovered in Ollama."""
...
def generate_with_ollama(prompt, context=None):
    """Generate AI response using local Ollama with READ-ONLY guardrails.

    Args:
        prompt: User question in Latvian
        context: Optional context from Developer tools (evidence results)

    Returns:
        dict with keys: "success", "text" or "error", "model"
    """
    # READ-ONLY SAFETY GUARD: Block any dangerous intent
    safe_prompt = str(prompt or "").lower()
    blocked_terms = [
        "izmaiņ","modificē","delete","remove","write ","creat ","edit ",
        "save ","store ","deploy ","commit ","push ","release ","implement ",
        "build ","compile","install","pip install","npm install","uninstall",
        "git add","git commit","git push","git merge","restart","stop",
        "execut ","run ","execute ","terminal","shell","command line",
    ]
    for term in blocked_terms:
        if term in safe_prompt:
            return {
                "success": False,
                "error": "AI ir READ-ONLY režīmā — netiek atļautas izmaiņu komandas.",
                "model": OLLAMA_MODEL_NAME,
            }

    try:
        # Build system prompt with strict READ-ONLY guardrails
        system_message = (
            "You are a READ-ONLY Latvian development assistant for NinaOS.\n\n"
            "RULES - STRICT:\n"
            "1. NEVER provide code that modifies files, database, git state, or deployment.\n"
            "2. ONLY answer from existing repository evidence provided in context.\n"
            "3. For 'where is' questions: return exact file + line number from context evidence.\n"
            "4. Never create new functions, classes, services, or frameworks not already in NinaOS.\n"
            "5. If evidence does not exist, say 'Nav atrasts esošā kodā' — never guess.\n"
            "6. All answers in Latvian unless code/identifiers.\n\n"
            "BOUNDARY: You are READ-ONLY analyst. User must use existing deterministic commands for any change."
        )
        ...
        if not context:
            full_prompt += "\n\nNO REPOSITORY EVIDENCE WAS PROVIDED. Do not claim to have inspected any file. Explain that source-backed diagnosis is unavailable."
        full_prompt += f"\n\nUSER QUESTION:\n{prompt}"
        ...
        with urllib.request.urlopen(req, timeout=AI_FALLBACK_TIMEOUT) as resp:
            result = json.loads(resp.read().decode("utf-8"))

        text = result.get("response", "").strip()
        if not text:
            return {"success": False, "error": "AI neatradīja atbildi.", "model": OLLAMA_MODEL_NAME}
        ...
        return {"success": False, "error": f"Ollama nav pieejams: {e.reason}", "model": OLLAMA_MODEL_NAME}
    except json.JSONDecodeError:
        return {"success": False, "error": "Nederīga AI atbilde.", "model": OLLAMA_MODEL_NAME}
    except Exception as e:
        return {"success": False, "error": f"AI kļūda: {e}", "model": OLLAMA_MODEL_NAME}


def ai_fallback_response(command, context_evidence=None):
    """Fallback for free-form Latvian questions after deterministic routing fails.

    Args:
        command: Original user question
        context_evidence: Optional evidence from Developer tools (globs/greps)

    Returns:
        dict with keys: "is_ai", "success", "text" or "error"
    """
    # First check if Ollama is available and has the model
    if not ollama_available():
        return {
            "is_ai": True,
            "success": False,
            "error": "AI nedarbojas: Ollama serviss nav pieejams (http://127.0.0.1:11434).",
        }

    if not ollama_model_discovered():
        return {
            "is_ai": True,
            "success": False,
            "error": f"AI nedarbojas: modelis {OLLAMA_MODEL_NAME} nav atrasts Ollama.",
        }

    # Generate response with context
    result = generate_with_ollama(command, context_evidence)
    result["is_ai"] = True
    return result
