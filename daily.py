...
# Core 2.5.2 polish: gala tekstā paliek tieši viena tīra Versija rinda.

REPLY_BUILDER_VERSION = "Core 2.5.2 — Reply Builder Polish"
...
def rb_clean_text(value):
    ...
    # Core 2.5.2: noņem VISAS vecās versiju rindas, arī moduļu footeri.
    # Piemēri:
    # Versija: Task Engine V1.2
    # Versija: Daily Brief / Work Inbox V1.0
    # Versija: V115.3 + Core 2.5.1
    text = re.sub(r"(?im)^\s*Versija:\s*.*$", "", text)

    # Sakārto tukšās rindas pēc footeru noņemšanas.
    text = re.sub(r"[ \t]+\n", "\n", text)
    text = re.sub(r"\n{3,}", "\n\n", text)
    return text.strip()
...
def build_reply_object(main_message="", user_text="", source="legacy_router", intent="", tone="", channel="telegram", metadata=None):
    return {
        ...
        "source": source or "legacy_router",
...
def reply_builder_build(reply_object):
    ...
    # Core 2.5.2 noteikums: gala tekstā vienmēr ir tieši viens NinaOS footer.
    text = text.rstrip() + f"\n\nVersija: {APP_VERSION}"
...
def reply_builder_status_answer():
    return reply_builder_text(
        "🧩 Core 2.5.2 — Reply Builder Polish ir aktīvs. ✅\n\n"
        "Gala atbildes tagad iet caur vienu centrālo komunikācijas slāni.\n\n"
        "Ko tas salabo:\n"
        "• noņem dubultās `Versija:` rindas;\n"
        "• patur moduļu saturu;\n"
        "• pieliek vienu tīru NinaOS footer;\n"
        "• neļauj labu moduļa atbildi pārrakstīt ar fallback.\n\n"
        "Tests:\n"
        "• klienti\n"
        "• kas notiek ar Andri\n"
        "• ko man tagad darīt\n"
