...
WORK_ENGINE_VERSION = "Work Engine V1.7 — ONE NINA Material Fact Grounding V1"
...
def build_grounded_material_acknowledgement(user_text: str, generator) -> Dict[str, Any]:
    """Summarize only facts explicitly present in work material.

    This is owner-facing. It must not invent quality judgments, merge separate facts,
    turn exterior parking into interior capacity, or ask follow-up questions unless the
    owner explicitly asked one.
    """
    text = _clean(user_text)
    if not text or generator is None:
        return {"ok": False, "text": ""}
...
Tev dots DARBA MATERIĀLS. Tavs uzdevums ir īsi apkopot tikai faktus, kas skaidri uzrakstīti pašā materiālā.

STINGRIE LIKUMI:
- Neizdomā nevienu faktu.
- Nevērtē ar vārdiem "labi kopts", "izdevīgs", "drošs", "piemērots", "ērts" vai līdzīgi, ja tas nav tieši rakstīts materiālā.
- Nejauc atsevišķus faktus kopā. Ja tekstā teikts, ka otru auto var novietot PIE VĀRTIEM, nesaki, ka garāža paredzēta DIVIEM AUTO.
- Neizdari secinājumus par ilgtermiņa lietošanu, drošību, tirgus cenu vai kvalitāti.
- Neveido pārdošanas tekstu.
- Neuzdod jautājumu beigās, ja lietotājs nav uzdevis jautājumu vai devis uzdevumu.
- Neraksti NinaOS, AI, versiju, Work Object vai citus tehniskus terminus.
- Atbildi latviski 2-3 īsos teikumos.
- Pirmajā teikumā pasaki, ko materiāls apraksta.
- Otrajā teikumā nosauc būtiskos faktus no teksta.
- Noslēgumā vari pateikt, ka materiāls ir piefiksēts darba kontekstā.
...
def build_grounded_photo_material_answer(user_text: str, vision_text: str, caption: str, generator) -> Dict[str, Any]:
    """Explain photo evidence against one work material without unsupported claims."""
    material = _clean(user_text)
    vision = _clean(vision_text)
    cap = _clean(caption)
    if not vision or generator is None:
        return {"ok": False, "text": ""}
    prompt = f"""
Tu esi Nina — uzņēmuma īpašnieka neredzamais darba palīgs.

Foto ir daļa no viena DARBA MATERIĀLA. Pasaki tikai to, ko foto droši parāda vai tieši apstiprina.

STINGRIE LIKUMI:
- Neizdomā neredzamus faktus.
- Neatkārto VISION ANALĪZES pārdošanas frāzes vai minējumus.
- Nevērtē kvalitāti, drošību, izdevīgumu vai piemērotību, ja to nevar tieši redzēt.
- Nejauc darba materiāla apgalvojumu ar foto pierādījumu. Ja teksts saka "sausa", foto pats to neapstiprina, ja vien tas nav skaidri redzams.
- Ja foto tikai rāda garāžas iekšpusi, saki tieši to.
- Neuzdod jautājumu beigās, ja lietotājs nav uzdevis jautājumu.
- Neraksti NinaOS, AI, versiju vai tehniskus terminus.
- Atbildi latviski 1-2 īsos teikumos.
...
VISION ANALĪZE (izmanto tikai kā izejmateriālu, nevis kā patiesību):
{vision[:3000]}
""".strip()
    try:
        result = _clean(generator(prompt))
