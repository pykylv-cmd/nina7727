...
try:
    ...
    from work_objects import canonical_client_conversation_thread as _canonical_client_conversation_thread
except ImportError:
    _append_client_conversation_turn = None
    _canonical_client_conversation_thread = None

try:
    from work_objects import save_client_workflow_decision as _save_client_workflow_decision
    from work_objects import canonical_client_workflow_state as _canonical_client_workflow_state
except ImportError:
    _save_client_workflow_decision = None
    _canonical_client_workflow_state = None
...
WORK_ENGINE_VERSION = "Work Engine V2.1 — ONE NINA Client Decision & Workflow Actions V1"
CLIENT_CONVERSATION_THREAD_ACTION_VERSION = "ONE_NINA_CLIENT_CONVERSATION_THREAD_ACTION_V1"
CLIENT_DECISION_ACTION_VERSION = "ONE_NINA_CLIENT_DECISION_WORKFLOW_ACTION_V1"
...
def read_client_conversation_thread(object_id: str) -> Dict[str, Any]:
    obj = get_work_object(object_id)
    if not obj:
        return {}
    if _canonical_client_conversation_thread is not None:
        return _canonical_client_conversation_thread(obj)
    metadata = obj.metadata if isinstance(getattr(obj, "metadata", None), dict) else {}
    thread = metadata.get("client_conversation_thread")
    return dict(thread) if isinstance(thread, dict) else {}


def persist_client_conversation_turn(
    object_id: str,
    *,
    question: str,
    answer: str,
    ...
    """Persist a grounded turn on the same canonical Work Object."""
    if _append_client_conversation_turn is not None:
        return _append_client_conversation_turn(
            object_id,
            question=question,
            answer=answer,
            ...
            action_version=action_version or CLIENT_CONVERSATION_THREAD_ACTION_VERSION,
            intake_version=intake_version,
            owner_forward_ready=owner_forward_ready,
        )

    # Staggered-deploy compatibility: keep the same metadata shape even when
    # work_objects.py has not yet reached V2.4. No second persistence is created.
    obj = get_work_object(object_id)
    if not obj:
        return None
    ...
    turns = list(thread.get("turns") or []) if isinstance(thread.get("turns"), list) else []
    if event_id and any(_clean(item.get("event_id")) == _clean(event_id) for item in turns if isinstance(item, dict)):
        return obj
    now = _utc_now()
    turn = {
        ...
        "action_version": _clean(action_version or CLIENT_CONVERSATION_THREAD_ACTION_VERSION),
        "intake_version": _clean(intake_version),
    ...
    thread.update({
        ...
        "version": "ONE_NINA_CLIENT_CONVERSATION_THREAD_V1",
        "object_id": obj.object_id,
        "client_id": _clean(getattr(obj, "client_id", "")),
        "turn_count": len(turns),
        "turns": turns,
        "latest_turn": turn,
        "latest_question": turn["question"],
    ...
    metadata["client_conversation_thread_version"] = "ONE_NINA_CLIENT_CONVERSATION_THREAD_V1"
    return update_work_object(obj.object_id, metadata=metadata)



def classify_client_decision(user_text: str) -> Dict[str, Any]:
    """Deterministically classify only explicit client workflow decisions."""
    raw = _clean(user_text)
    text = _fold_text(raw)
    if not text or "?" in raw:
        return {"matched": False, "decision": "", "reason": "empty_or_question"}
    groups = (
        ("client_requests_changes", (r"\bgribu\s+(?:vel\s+)?izmain", r"\bvajag\s+(?:ko\s+)?main", r"\bludzu\s+main", r"\bveletos\s+izmain", r"\bvelos\s+izmain", r"\bprecizejam\b", r"\bkorige", r"\bparstrada")),
        ("client_rejected", (r"\bneapstiprinu\b", r"\bnoraidu\b", r"\batsakos\b", r"\bnepiekritu\b", r"\bnepienem", r"\bneinterese\b", r"\batteiksimies\b")),
        ("client_approved", (r"\bapstiprinu\b", r"\bapstiprinam\b", r"\bpiekritu\b", r"\bpienemu\b", r"\bder\s*,?\s*daram\b", r"\bvaram\s+sakt\b", r"\bsakam\s+darbus\b", r"\btame\s+der\b", r"\bpiedavajums\s+der\b")),
        ("client_needs_followup", (r"\batsutisu\s+vel\b", r"\bdosu\s+zinu\b", r"\batgriezisos\b", r"\bjapadoma\b", r"\bvajag\s+padomat\b", r"\bsazinasos\b")),
    )
    for decision, patterns in groups:
        for pattern in patterns:
            if re.search(pattern, text, flags=re.IGNORECASE):
                return {"matched": True, "decision": decision, "evidence_text": raw, "confidence": "explicit", "reason": "deterministic_explicit_client_decision", "action_version": CLIENT_DECISION_ACTION_VERSION}
    return {"matched": False, "decision": "", "reason": "no_explicit_decision"}


def _client_decision_next_action(decision: str) -> str:
    return {
        "client_approved": "Vienoties ar klientu par darbu sākšanas datumu un nākamo organizatorisko soli.",
        "client_rejected": "Pārskatīt atteikuma iemeslu un izlemt, vai vajadzīgs korekts follow-up.",
        "client_requests_changes": "Pārskatīt klienta prasītās izmaiņas un sagatavot atjaunotu tāmes vai piedāvājuma versiju.",
        "client_needs_followup": "Atgriezties pie klienta pēc viņa norādītā vai saprātīga follow-up termiņa.",
    }.get(_clean(decision), "Pārskatīt klienta lēmumu un noteikt nākamo darba soli.")


def apply_client_decision_to_work_object(object_id: str, *, client_text: str, sender_name: str = "", channel: str = "", event_id: str = "") -> Dict[str, Any]:
    obj = get_work_object(object_id)
    if not obj:
        return {"ok": False, "matched": False, "error": "work_object_not_found", "object_id": object_id}
    classified = classify_client_decision(client_text)
    if not classified.get("matched"):
        return {"ok": False, "matched": False, "error": classified.get("reason", "not_a_decision"), "object_id": object_id}
    decision = _clean(classified.get("decision"))
    next_action = _client_decision_next_action(decision)
    if _save_client_workflow_decision is not None:
        saved = _save_client_workflow_decision(object_id, decision=decision, evidence_text=_clean(classified.get("evidence_text") or client_text), sender_name=sender_name, channel=channel, event_id=event_id, owner_next_action=next_action, confidence=_clean(classified.get("confidence") or "explicit"))
    else:
        current = get_work_object(object_id)
        if not current:
            saved = None
        else:
            ...
            if event_id and any(_clean(item.get("event_id")) == _clean(event_id) for item in events if isinstance(item, dict)):
                saved = current
            else:
                event = {"event_index": len(events)+1, "decision": decision, "evidence_text": _clean(client_text), "sender_name": _clean(sender_name), "channel": _clean(channel), "event_id": _clean(event_id), "confidence": "explicit", "owner_next_action": next_action, "created_at": now}
                events.append(event)
                state.update({"version": "ONE_NINA_CLIENT_DECISION_WORKFLOW_V1", "object_id": current.object_id, "decision_state": decision, "event_count": len(events[-100:]), "events": events[-100:], "latest_event": event, "latest_evidence_text": _clean(client_text), "owner_next_action": next_action, "last_activity_at": now})
                metadata["client_workflow_state"] = state
                metadata["latest_client_decision"] = event
                saved = update_work_object(current.object_id, metadata=metadata)
    if not saved:
        return {"ok": False, "matched": True, "error": "client_decision_persistence_failed", "object_id": object_id}
    client_label = _clean(sender_name) or "Klients"
    owner_text = {
        "client_approved": f"{client_label} apstiprināja tāmi. Lēmumu piefiksēju pie šī paša darba. Nākamais solis — vienoties par darbu sākšanas datumu.",
        "client_rejected": f"{client_label} atteicās no tāmes. Lēmumu piefiksēju pie šī paša darba. Nākamais solis — pārskatīt atteikuma iemeslu un izlemt par follow-up.",
        "client_requests_changes": f"{client_label} vēlas izmaiņas. Pieprasījumu piefiksēju pie šī paša darba. Nākamais solis — pārskatīt izmaiņas un sagatavot atjaunotu tāmi.",
        "client_needs_followup": f"{client_label} vēl nav devis gala lēmumu. Follow-up vajadzību piefiksēju pie šī paša darba. Nākamais solis — atgriezties pie klienta vēlāk.",
    }.get(decision, f"{client_label} lēmumu piefiksēju pie šī paša darba.")
    return {"ok": True, "matched": True, "handled": True, "action": "client_decision", "action_version": CLIENT_DECISION_ACTION_VERSION, "object_id": object_id, "decision": decision, "evidence_text": _clean(classified.get("evidence_text") or client_text), "owner_next_action": next_action, "text": owner_text}
