
# NINA_ROADMAP.md

# NinaOS Roadmap

## Constitution V6 alignment

The authoritative direction is `NinaOS_Constitution_V6.md`: customers speak
with Nina, and Nina coordinates permitted ready workers, providers, systems,
channels, people, and future physical workers. Customers receive ready workers;
they are not required to build bots.

The immediate architecture sequence is Platform Core V1 (implemented),
RolePack System V1 (implemented), Ready Worker Catalog V1 (implemented), then
**Agent Assignment V1 (next)**. The older phase roadmap below is retained as
historical product context where it does not conflict with that sequence.

WhatsApp is no longer merely future: Meta Cloud API and Personal/Company
WhatsApp paths exist at partial status. The complete provider-neutral Channel
Layer remains planned. OpenAI is the evidenced runtime AI provider; the
multi-provider Hub is strategic. Voice is experimental/partial. Robot and
humanoid orchestration is strategic future architecture with no current
physical-control implementation.

Version: 1.0
Status: Active

---

# NORTH STAR

Izveidot pasaules AI operētājsistēmu, uz kuras darbojas miljoniem AI darbinieku cilvēkiem un uzņēmumiem.

Telegram ir tikai pirmais testa adapters.

---

# PHASE 1 — Stabils kodols

## Mērķis
Uzbūvēt uzticamu Nina Core.

### Core Evolution
- ✅ 2.0 Employee Brain
- ✅ 2.1 Identity
- ✅ 2.2 Responsibility
- ✅ 2.3 Think
- ✅ 2.4 Learning
- ✅ 2.5 Quality
- ✅ 2.5.1 Reply Builder
- 🚧 2.6 Initiative
- ⏳ 2.7 Context
- ⏳ 2.8 Memory Intelligence
- ⏳ 3.0 Employee Brain v3

Panākumu kritērijs:
Core komandas vienmēr strādā un neiet pa vecajiem maršrutiem.

---

# PHASE 2 — AI Darbinieki

Katram AI:
- identitāte
- atmiņa
- prasmes
- reputācija
- instrumenti
- darba vēsture

Pirmie:
- Nina
- Tāmētājs
- Grāmatvedis
- Jurists
- Pārdošanas AI
- CEO Assistant

---

# PHASE 3 — Enterprise

Automatizēt:
- e-pastus
- kalendāru
- CRM
- dokumentus
- tāmēšanu
- grāmatvedību
- klientu komunikāciju
- projektu vadību

---

# PHASE 4 — Nina Exchange

AI tirgus:
- AI darbinieku noma
- AI pakalpojumi
- AI moduļu veikals
- Promptu veikals
- Automatizāciju veikals
- Uzņēmumu risinājumi

Ieņēmumi:
- komisijas
- abonementi
- uzņēmumu licences

---

# PHASE 5 — Nina Pay

Norēķini:
- abonementi
- komisijas
- partneru izmaksas
- AI autoratlīdzības

---

# PHASE 6 — AI Factory

Izveidot sistēmu, kas rada jaunus AI darbiniekus.

Process:
1. izvēlies profesiju
2. piešķir prasmes
3. pieslēdz instrumentus
4. pievieno atmiņu
5. publicē Exchange

---

# PHASE 7 — Platforma

Kanāli:
- Telegram
- Web
- Mobile
- API
- Voice
- Discord
- WhatsApp
- Roboti/IoT

---

# 3 gadu mērķi

- Stabils NinaOS Core
- AI Factory
- Nina Exchange
- 100+ AI darbinieku tipi
- Enterprise klienti
- Starptautiska platforma

---

# Darba princips

Jauna funkcija netiek būvēta, kamēr:
1. nav noteikts modulis;
2. nav atjaunināta dokumentācija;
3. nav definēts tests;
4. nav skaidrs, kā tā palīdz NinaOS ilgtermiņa mērķim.
