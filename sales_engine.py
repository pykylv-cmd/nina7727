...
def nina_parse_relationship_facts_from_user(user):
    """
    Backup bridge.
    Ja relationship_engine tabulas/atmiņas nav atrodamas, paņemam attiecības no users.facts.
    Piemērs: Andris → client; Anna → wife; Reksis → dog
    """
...
def nina_profile_summary_v11(user_id):
    ...
    # Kritiskais labojums: ja relationship_engine atmiņa tukša, lasām arī users.facts.
    fact_rels = nina_parse_relationship_facts_from_user(user)
    rels = (rels or []) + fact_rels
    ...
    if len(lines) == 1:
        # Extra fallback: show raw facts if nothing else exists.
    ...
    lines.append("Ja kaut kas nav pareizi, pasaki tieši — es labošu profilu, nevis strīdēšos.")
    lines.append("")
    lines.append("Versija: Profile Summary V1.2")
    return "\n".join(lines)
