...
def subscription_info(user_id=None):
    """V13.2: Premium pārdošanas teksts ar cilvēkam saprotamu vērtību."""
...
def premium_conversion_answer(user_id):
    """V13.2: labāks Free -> Premium pārdošanas teksts."""
    user = get_user(user_id)
    if user.get("premium"):
...
# =========================
# V13.2 NATURAL MEMORY + DAILY GOALS
# =========================
...
def nina_daily_habit_answer(user_id):
    """V13.2: Daily Assistant ar šodienas mērķiem un dabiskāku tekstu."""
    user = get_user(str(user_id))
...
def nina_evening_answer(user_id):
    """V13.2: vakara kopsavilkuma sagatave."""
    return (
...
async def reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # V13.2: Natural memory and daily goal capture.
    if lower.startswith("atceries,") or lower.startswith("atceries ka") or lower.startswith("atceries "):
        ...
        if saved:
            await update.message.reply_text(
                append_bonus_notices(nina_memory_saved_answer(saved), streak_notice),
    ...
    if lower.startswith("mērķis:") or lower.startswith("merkis:") or lower.startswith("šodienas mērķis:") or lower.startswith("sodienas merkis:"):
        ...
        if goal_text:
            ...
            await update.message.reply_text(
                append_bonus_notices(nina_goal_saved_answer(goal_text), streak_notice),
                disable_web_page_preview=True
            )
            return
    ...
    # V13.2: Daily Assistant extra commands.
...
@app.route("/")
def home():
    return "Nina7727 V13.2 Premium Sales Text darbojas! DB: " + ("PostgreSQL" if USE_POSTGRES else "SQLite fallback")
...
if __name__ == "__main__":
    print("Nina7727 V13.2 Premium Sales Text darbojas...", "PostgreSQL" if USE_POSTGRES else "SQLite fallback")
