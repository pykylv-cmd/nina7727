...
from assistant import (
    build_start_answer,
    build_daily_habit_answer,
    build_remember_prompt_answer,
    build_invite_answer,
)
...
def subscription_info(user_id=None):
    """V12.6.1: Premium pārdošanas teksts ar cilvēkam saprotamu vērtību."""
...
def premium_conversion_answer(user_id):
    """V12.6.1: labāks Free -> Premium pārdošanas teksts."""
    user = get_user(user_id)
    if user.get("premium"):
...
def nina_start_answer(user_id=None):
    """V13.0.1: teksts pārvietots uz assistant.py."""
    return build_start_answer(version="V13.0.1")



def nina_daily_habit_answer(user_id):
    """V13.0.1: 'mana diena' teksts pārvietots uz assistant.py."""
    user = get_user(str(user_id))
    ...
    return build_daily_habit_answer(
        name=name,
        plan=plan,
        reminders=reminders,
        backups=backups,
...
def nina_remember_prompt_answer(user_id=None):
    """V13.0.1: 'atceries' teksts pārvietots uz assistant.py."""
    return build_remember_prompt_answer(version="V13.0.1")



def nina_launch_invite_text(user_id):
    """V13.0.1: invite teksts pārvietots uz assistant.py."""
    bot_username = os.environ.get("TELEGRAM_BOT_USERNAME", "Nina7727_bot")
    code = referral_code_for_user(user_id) if "referral_code_for_user" in globals() else f"NINA-{user_id}"
    link = f"https://t.me/{bot_username}?start={code}"
    return build_invite_answer(link=link, version="V13.0.1")
...
@app.route("/")
def home():
    return "Nina7727 V12.6.1 Premium Sales Text darbojas! DB: " + ("PostgreSQL" if USE_POSTGRES else "SQLite fallback")
...
if __name__ == "__main__":
    print("Nina7727 V12.6.1 Premium Sales Text darbojas...", "PostgreSQL" if USE_POSTGRES else "SQLite fallback")
