...
APP_VERSION = "V118.1.9 + ONE NINA Document Inclusion Follow-up Gate Fix V1"
...
def nina_is_document_followup_question(user_text):
    ...
    if not text or "?" not in text and not any(
        text.startswith(x) for x in ("cik ", "kas ", "kāda ", "kads ", "kad ", "kur ", "kam ", "vai ")
    ):
        return False
    ...
    if any(ref in text for ref in document_refs):
        return True

    # V118.1.9 — natural continuation questions often omit the repeated words
    # “tāme” or “dokuments”. Inclusion questions are document-grounded when a
    # canonical document exists; the next function safely returns None otherwise.
    inclusion_markers = (
        "ir iekļauts", "ir iekļauta", "ir iekļauti", "ir iekļautas",
        "vai iekļauts", "vai iekļauta", "vai ietilpst",
        "ieklauts", "ieklauta", "ietilpst",
    )
    return text.startswith("vai ") and any(marker in text for marker in inclusion_markers)
        ...
        if ONE_NINA_DOCUMENT_INTAKE_READY:
            ...
            if document_action_match.get("matched"):
                ...
                if document_action and str(document_action.get("answer") or "").strip():
                    ...
                    # V118.1.7 — this branch is terminal and previously returned
                    # before preserving the exact canonical Work Object selected by
                    # the document action. Store only a temporary routing pointer;
                    # all business truth remains in nina_work_objects.
                    action_obj = document_action.get("object")
                    if action_obj is not None and str(getattr(action_obj, "object_id", "") or "").strip():
                        context.chat_data["one_nina_active_work_object_id"] = str(action_obj.object_id)
                        context.chat_data["one_nina_active_work_object_at"] = datetime.now(timezone.utc).isoformat()
                        try:
                            saved_active_obj = nina_mark_active_document_context(
                                action_obj,
                                user_id=user_id,
                                interaction_type="canonical_document_work_action",
                                user_text=user_text,
                                answer=answer,
                            )
                            if saved_active_obj is not None:
                                action_obj = saved_active_obj
                        except Exception as e:
                            print("ONE NINA document action active context save error:", repr(e))
