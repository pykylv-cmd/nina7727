...
def subscription_info(user_id=None):
    """V13.2.1: Premium pārdošanas teksts ar cilvēkam saprotamu vērtību."""
...
def premium_conversion_answer(user_id):
    """V13.2.1: labāks Free -> Premium pārdošanas teksts."""
    user = get_user(user_id)
    if user.get("premium"):
...
# =========================
# V13.2.1 NATURAL MEMORY + DAILY GOALS
# =========================

def save_daily_goal(user_id, goal_text):
    """Saglabā šodienas mērķi."""
...
def latest_daily_goals(user_id, limit=3):
    """Atgriež pēdējos šodienas mērķus."""
    today = datetime.now(ZoneInfo(DEFAULT_TIMEZONE)).strftime("%Y-%m-%d")
    ...
    try:
        ...
        db_execute(
            ...
            LIMIT %s
            """,
            (str(user_id), today, int(limit or 3))
        )
        rows = c.fetchall()
        c.close()
        conn.close()
...
def nina_memory_saved_answer(saved_text):
    return (
        "🧠 Pierakstīju. ✅\n\n"
        f"Es atcerēšos: {saved_text}\n\n"
        "Vēlāk vari man prasīt vai papildināt šo domu.\n\n"
        "Versija: V13.2.1"
    )
...
def nina_goal_saved_answer(goal_text):
    return (
        "🎯 Šodienas mērķis pierakstīts. ✅\n\n"
        f"Mērķis: {goal_text}\n\n"
        "Tagad dienai ir skaidrs virziens. Ja gribi, vari pastāstīt, kas būs pirmais mazais solis.\n\n"
        "Versija: V13.2.1"
    )


def nina_daily_habit_answer(user_id):
    """V13.2.1: Daily Assistant ar šodienas mērķiem un dabiskāku tekstu."""
    user = get_user(str(user_id))
    ...
    try:
        backups = backup_count_number(str(user_id))
    except Exception:
        backups = 0

    goals = latest_daily_goals(str(user_id), limit=3)
    ...
    if reminders == 0:
        reminder_text = "Šobrīd tev nav aktīvu atgādinājumu. Varam mierīgi saplānot svarīgāko."
    else:
    ...
    if goals:
        goal_lines = ["Šodienas mērķi:"]
        for i, row in enumerate(goals, start=1):
            goal_lines.append(f"{i}. {row[0]}")
        goals_text = "\n".join(goal_lines)
    else:
    ...
    return (
        ...
        f"Plāns: {plan}\n\n"
        "Ko darām tālāk?\n"
        "• uzraksti: mērķis: tavs mērķis\n"
        "• uzraksti: atceries, ka...\n"
...
def nina_evening_answer(user_id):
    """V13.2.1: vakara kopsavilkuma sagatave."""
    return (
...
def nina_today_goal_answer(user_id):
    """V13.2.1: šodienas mērķa ievade."""
    return (
        "🎯 Šodienas mērķis\n\n"
        "Uzraksti vienu galveno lietu, ko šodien gribi paveikt.\n\n"
        "Piemēram:\n"
        "Šodienas mērķis: piezvanīt klientam un pabeigt piedāvājumu.\n\n"
...
async def reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # V13.2.1: Natural memory and daily goal capture.
    # Šis bloks apzināti nesauc append_bonus_notices(), lai atbildes nepazūd,
    # ja streak_notice vēl nav izveidots šajā vietā.
    if lower.startswith("atceries,") or lower.startswith("atceries ka") or lower.startswith("atceries "):
        ...
        if saved:
            await update.message.reply_text(
                nina_memory_saved_answer(saved),
    ...
    if lower.startswith("mērķis:") or lower.startswith("merkis:") or lower.startswith("šodienas mērķis:") or lower.startswith("sodienas merkis:"):
        ...
        if goal_text:
            save_daily_goal(user_id, goal_text)
            await update.message.reply_text(
                nina_goal_saved_answer(goal_text),
                disable_web_page_preview=True
            )
            return
...
@app.route("/")
def home():
    return "Nina7727 V13.2.1 Premium Sales Text darbojas! DB: " + ("PostgreSQL" if USE_POSTGRES else "SQLite fallback")
...
if __name__ == "__main__":
    print("Nina7727 V13.2.1 Premium Sales Text darbojas...", "PostgreSQL" if USE_POSTGRES else "SQLite fallback")
