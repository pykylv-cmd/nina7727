...
def subscription_info(user_id=None):
    """V11.8.1 Stripe Test Router Fix — skaidrs abonementa piedāvājums Free lietotājiem."""
...
def stripe_checkout_answer(user_id, plan_key="basic"):
    """V11.8.1 Stripe Test Router Fix — izveido reālu Stripe Checkout linku, ja ENV ir pieslēgts."""
    if plan_key == "plus":
        plan_name = PLAN_PREMIUM_PLUS
        amount = PREMIUM_PLUS_PRICE
        price_id = STRIPE_PLUS_PRICE_ID
...
def stripe_env_guide_answer(user_id=None):
    """V11.8.1 Stripe Test Router Fix — atsevišķs Stripe ENV panelis."""
    checks = [
...
def stripe_webhook_test_answer(user_id):
    """V11.8.1 Stripe Test Router Fix — testē Premium aktivizāciju bez īsta Stripe maksājuma."""
...
async def reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # V11.8.1 HARD FIX: catch Stripe test before GPT fallback.
...
def activate_premium_from_stripe(user_id, plan_key="basic", stripe_session_id="", stripe_event_id="", customer_email=""):
    """V11.8.1 Stripe Test Router Fix — ieslēdz Premium pēc Stripe maksājuma."""
    if not user_id:
...
def stripe_webhook():
    """V11.8.1 Stripe Test Router Fix."""
    if not stripe:
...
@app.route("/")
def home():
    return "Nina7727 V11.8.1 Stripe Test Router Fix darbojas! DB: " + ("PostgreSQL" if USE_POSTGRES else "SQLite fallback")
...
if __name__ == "__main__":
    print("Nina7727 V11.8.1 Stripe Test Router Fix darbojas...", "PostgreSQL" if USE_POSTGRES else "SQLite fallback")
