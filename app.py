...
def subscription_info(user_id=None):
    """V14.0: Premium pārdošanas teksts ar cilvēkam saprotamu vērtību."""
...
def premium_conversion_answer(user_id):
    """V14.0: labāks Free -> Premium pārdošanas teksts."""
    user = get_user(user_id)
    if user.get("premium"):
...
# =========================
# V14.0 NATURAL MEMORY + DAILY GOALS
# =========================

def save_daily_goal(user_id, goal_text):
    """V14.0: Saglabā šodienas mērķi, aizvietojot veco šodienas mērķi."""
...
def latest_daily_goals(user_id, limit=3):
    """V14.0: Atgriež šodienas aktīvo mērķi/mērķus."""
    today = datetime.now(ZoneInfo(DEFAULT_TIMEZONE)).strftime("%Y-%m-%d")
...
def latest_natural_memories(user_id, limit=3):
    """V14.0: Droši atgriež pēdējās svarīgās lietas."""
    try:
        conn = get_db()
        c = conn.cursor()
        db_execute(
...
def nina_daily_habit_answer(user_id):
    """V14.0: Daily Assistant loģika paliek app.py, teksti pārvietoti uz daily.py."""
    try:
        user = get_user(str(user_id))
    except Exception as e:
...
def nina_morning_answer(user_id):
    """V14.0: labrīta teksts no daily.py."""
    try:
        user = get_user(str(user_id))
        name = (user.get("name") or "").strip()
    except Exception:
        name = ""
    return build_morning_answer(name=name, version="V14.0")



def nina_evening_answer(user_id):
    """V14.0: vakara teksts no daily.py."""
    return build_evening_answer(version="V14.0")



def nina_today_goal_answer(user_id):
    """V14.0: mērķa teksta sagatave no daily.py."""
    return build_goal_prompt_answer(version="V14.0")
...
async def reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # V14.0: Mana diena top-priority route.
    ...
    # V14.0: Natural memory and daily goal capture — immediate replies.
...
@app.route("/")
def home():
    return "Nina7727 V14.0 Premium Sales Text darbojas! DB: " + ("PostgreSQL" if USE_POSTGRES else "SQLite fallback")
...
if __name__ == "__main__":
    print("Nina7727 V14.0 Premium Sales Text darbojas...", "PostgreSQL" if USE_POSTGRES else "SQLite fallback")
