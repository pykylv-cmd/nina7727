...
def subscription_info(user_id=None):
    """V11.8 Stripe Webhook Test Mode — skaidrs abonementa piedāvājums Free lietotājiem."""
...
def stripe_checkout_answer(user_id, plan_key="basic"):
    """V11.8 Stripe Webhook Test Mode — izveido reālu Stripe Checkout linku, ja ENV ir pieslēgts."""
    if plan_key == "plus":
        plan_name = PLAN_PREMIUM_PLUS
        amount = PREMIUM_PLUS_PRICE
        price_id = STRIPE_PLUS_PRICE_ID
...
def stripe_env_guide_answer(user_id=None):
    """V11.8 Stripe Webhook Test Mode — atsevišķs Stripe ENV panelis."""
    checks = [
...
def stripe_webhook_test_answer(user_id):
    """V11.8 Stripe Webhook Test Mode — testē Premium aktivizāciju bez īsta Stripe maksājuma."""
    ok, result = activate_premium_from_stripe(
        user_id=str(user_id),
        plan_key="basic",
        stripe_session_id="test_session_v11_8",
        stripe_event_id="test_event_v11_8",
...
def activate_premium_from_stripe(user_id, plan_key="basic", stripe_session_id="", stripe_event_id="", customer_email=""):
    """V11.8 Stripe Webhook Test Mode — ieslēdz Premium pēc Stripe maksājuma."""
    if not user_id:
...
def stripe_webhook():
    """V11.8 Stripe Webhook Test Mode."""
    if not stripe:
...
@app.route("/")
def home():
    return "Nina7727 V11.8 Stripe Webhook Test Mode darbojas! DB: " + ("PostgreSQL" if USE_POSTGRES else "SQLite fallback")
...
if __name__ == "__main__":
    print("Nina7727 V11.8 Stripe Webhook Test Mode darbojas...", "PostgreSQL" if USE_POSTGRES else "SQLite fallback")
