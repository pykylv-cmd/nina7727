...
# V14.3 Safe Daily Module Import
...
try:
    ...
    def build_daily_answer(name="", plan="Free", is_premium=False, goals=None, memories=None, reminders=0, version="V14.3"):
        goals = goals or []
    ...
    def build_morning_answer(name="", version="V14.3"):
    ...
    def build_evening_answer(version="V14.3"):
        return (
    ...
    def build_goal_prompt_answer(version="V14.3"):
        return (
            "🎯 Šodienas mērķis\n\n"
...
# V14.3 Safe Memory Module Import
...
try:
    ...
    def build_memory_saved_answer(saved_text, version="V14.3"):
        return (
            "🧠 Pierakstīju. ✅\n\n"
    ...
    def build_goal_saved_answer(goal_text, version="V14.3"):
        return (
...
def subscription_info(user_id=None):
    """V14.3: Premium pārdošanas teksts ar cilvēkam saprotamu vērtību."""
...
def premium_conversion_answer(user_id):
    """V14.3: labāks Free -> Premium pārdošanas teksts."""
    user = get_user(user_id)
    if user.get("premium"):
...
def save_natural_memory(user_id, memory_text):
    """V14.3: dabiskās atmiņas saglabāšana pārvietota uz memory.py loģiku."""
    return save_natural_memory_logic(get_db, db_execute, user_id, memory_text)



def latest_natural_memories(user_id, limit=3):
    """V14.3: pēdējo atmiņu nolasīšana pārvietota uz memory.py loģiku."""
    return latest_natural_memories_logic(get_db, db_execute, user_id, limit)



def nina_memory_saved_answer(saved_text):
    """V14.3: atmiņas saglabāšanas teksts no memory.py vai fallback."""
    return build_memory_saved_answer(saved_text, version="V14.3")



def nina_goal_saved_answer(goal_text):
    """V14.3: mērķa saglabāšanas teksts no memory.py vai fallback."""
    return build_goal_saved_answer(goal_text, version="V14.3")



def nina_daily_habit_answer(user_id):
    """V14.3: Daily Assistant loģika paliek app.py, teksti no daily.py vai fallback."""
    try:
        user = get_user(str(user_id))
    except Exception as e:
...
def nina_morning_answer(user_id):
    """V14.3: labrīta teksts no daily.py vai fallback."""
    try:
        user = get_user(str(user_id))
        name = (user.get("name") or "").strip()
    except Exception:
        name = ""
    return build_morning_answer(name=name, version="V14.3")



def nina_evening_answer(user_id):
    """V14.3: vakara teksts no daily.py vai fallback."""
    return build_evening_answer(version="V14.3")



def nina_today_goal_answer(user_id):
    """V14.3: mērķa teksta sagatave no daily.py vai fallback."""
    return build_goal_prompt_answer(version="V14.3")
...
# =========================
# V14.3 NATURAL ASSISTANT PROMPT
# =========================

def looks_like_action_or_plan(text):
    """Atpazīst dabisku frāzi, kas izklausās pēc plāna, uzdevuma vai lietas, ko vajag atcerēties."""
    lower = (text or "").strip().lower()

    if not lower:
        return False

    # Neķeram jau esošās komandas.
    blocked_starts = [
    ...
    keywords = [
        "rīt", "rit", "šodien", "sodien", "pirmdien", "otrdien", "trešdien", "tresdien",
        "ceturtdien", "piektdien", "sestdien", "svētdien", "svetdien",
        "jāzvana", "jazvana", "jāizdara", "jaizdara", "jānopērk", "janoperk",
        "neaizmirst", "atgādini", "atgadini", "vajag", "jāsatiek", "jasatiek",
        "tikšanās", "tiksanas", "klientam", "ārsts", "arsts", "zobārsts", "zobarsts"
    ]

    return any(k in lower for k in keywords)


def nina_natural_assistant_offer(user_text):
    return (
        "Saprotu. Tas izklausās pēc lietas, ko varētu būt vērts nepazaudēt.\\n\\n"
        f"Tu rakstīji: {user_text}\\n\\n"
        "Ko vēlies, lai es ar to daru?\\n"
        "1. Raksti: atceries, ka " + user_text + "\\n"
        "2. Raksti: mērķis: " + user_text + "\\n"
        "3. Vai vienkārši pastāsti man vairāk, un es palīdzēšu sakārtot domu.\\n\\n"
        "Versija: V14.3"
    )
...
async def reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # V14.3: Mana diena top-priority route.
    ...
    # V14.3: Natural memory and daily goal capture — immediate replies.
    ...
    # V14.3: Natural assistant prompt.
    if looks_like_action_or_plan(user_text):
        await update.message.reply_text(
            nina_natural_assistant_offer(user_text),
            disable_web_page_preview=True
        )
        return
...
@app.route("/")
def home():
    return "Nina7727 V14.3 Premium Sales Text darbojas! DB: " + ("PostgreSQL" if USE_POSTGRES else "SQLite fallback")
...
if __name__ == "__main__":
    print("Nina7727 V14.3 Premium Sales Text darbojas...", "PostgreSQL" if USE_POSTGRES else "SQLite fallback")
