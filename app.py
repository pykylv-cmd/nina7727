...
# V30.1 Safe User Profile Engine Import
try:
    ...
    def build_profile_saved_answer(fact_type, value, version="V30.1"):
        if fact_type == "name":
...
# V30.1 Safe Vision Engine Import
try:
    ...
    def build_vision_answer_from_openai(client, image_bytes, caption="", version="V30.1"):
        return build_no_vision_fallback(version=version)

    def build_no_vision_fallback(version="V30.1"):
        return (
...
# V30.1 Safe Daily Module Import
...
try:
    ...
    def build_daily_answer(name="", plan="Free", is_premium=False, goals=None, memories=None, reminders=0, version="V30.1"):
        goals = goals or []
    ...
    def build_morning_answer(name="", version="V30.1"):
    ...
    def build_evening_answer(version="V30.1"):
        return (
    ...
    def build_goal_prompt_answer(version="V30.1"):
        return (
            "🎯 Šodienas mērķis\n\n"
...
# V30.1 Safe Memory Module Import
...
try:
    ...
    def build_memory_saved_answer(saved_text, version="V30.1"):
        return (
            "🧠 Pierakstīju. ✅\n\n"
    ...
    def build_goal_saved_answer(goal_text, version="V30.1"):
        return (
...
# V30.1 Safe Conversation Module Import
...
try:
    ...
    def build_auto_memory_answer(memory_text, version="V30.1"):
        return (
            "🧠 Saglabāju. ✅\n\n"
    ...
    def build_auto_goal_answer(goal_text, version="V30.1"):
        return (
...
# V30.1 Safe Coach Module Import
try:
...
# V30.1 Safe Personality Module Import
try:
...
# V30.1 Safe Reminders Module Import
...
try:
    ...
    def build_reminder_saved_answer(reminder_text, human_time="", version="V30.1"):
        if human_time:
            return (
    ...
    def build_reminder_help_answer(version="V30.1"):
        return (
            "⏰ Raksti šādi:\n\n"
...
# V30.1 Safe Brain Module Import
try:
...
# V30.1 Safe Analytics Module Import
...
try:
    ...
    def build_weekly_progress_text(snapshot, topic_counts=None, version="V30.1"):
        return (
    ...
    def build_empty_progress_text(version="V30.1"):
        return (
            "📊 Tavs progress ar Ninu\n\n"
...
# V30.1 Safe Dialog Module Import
...
try:
    ...
    def build_capabilities_answer(version="V30.1"):
        return (
    ...
    def build_playful_rough_answer(version="V30.1"):
        return (
    ...
    def build_smalltalk_answer(user_text="", version="V30.1"):
        return (
            "Esmu te. 😊\n\n"
...
# V30.1 Safe Charm Module Import
try:
    from charm import (
    ...
    def charm_capabilities_answer(version="V30.1"):
        return (
    ...
    def charm_smalltalk_answer(user_text="", version="V30.1"):
        return (
            "Čau. 😊 Esmu te.\n\n"
    ...
    def charm_rough_answer(version="V30.1"):
        return (
...
# V30.1 Safe Persona Engine Import
try:
...
# V30.1 Living Conversation Core
...
def v18_should_use_human_mode(text):
    """V30.1: nosaka, kad Nina runā kā cilvēks, nevis ar vecām robota atbildēm."""
    lower = (text or "").strip().lower()
    if not lower:
        return True
...
def v18_human_mode_answer(text):
    """V30.1 Human Mode: fokusējas uz cilvēku, emociju un nākamo ziņu."""
    lower = (text or "").strip().lower()
...
def v18_human_capabilities_answer():
    """V30.1: īpaši cilvēcisks teksts jautājumam 'ko vari darīt?'."""
    try:
        import random
        variants = [
            (
...
def save_conversation_state(user_id, user_text, nina_text="", intent="", emotion="", topic=""):
    """V30.1: saglabā īstermiņa sarunas kontekstu."""
    try:
        conn = get_db()
        c = conn.cursor()
        db_execute(
...
def latest_conversation_state(user_id, limit=3):
    """V30.1: nolasa pēdējo sarunas kontekstu."""
    try:
        conn = get_db()
        c = conn.cursor()
        db_execute(
...
def v19_human_mode_answer_with_memory(user_id, text):
    """V30.1: Human Mode + īstermiņa sarunas atmiņa."""
    lower = (text or "").strip().lower()
    emotion = detect_emotion(text)
    topic = detect_topic(text)
...
def v20_smart_price_answer(user_id=None):
    """V30.1: atbild uz cenu/tarifu jautājumiem cilvēciski un komerciāli."""
    try:
...
def v21_memory_answer(memory_text,version="V30.1"):
    return (
        "🧠 Saglabāju. ✅\n\n"
        f"Atcerēšos: {memory_text}\n\n"
        "💬 Vai gribi, lai es tev par to arī atgādinu īstajā laikā?\n"
...
def v21_build_memory_answer(memory_text, version="V30.1"):
    memory_text = (memory_text or "").strip()
...
def v24_save_profile_fact_to_db(user_id, fact_type, value):
    """V30.1: saglabā profila faktu users tabulā, lai Nina to atceras pēc restartēšanas."""
...
def v24_profile_answer_from_fact(user_id, user_text):
    """V30.1: atpazīst profila faktus un uzreiz saglabā datubāzē."""
    ...
    return build_profile_saved_answer(fact_type, value, version="V30.1")


def v24_profile_recall_answer(user_id):
    """V30.1: parāda, ko Nina jau zina par lietotāju."""
    user = get_user(str(user_id))
...
def v301_capabilities(user_id):
    ...
    lines += ["", "Tev nav jāiegaumē komandas. Raksti normāli, piemēram:", "rīt jāzvana klientam", "man šodien daudz darbu", "apskati šo bildi", "", "Versija: V30.1"]
    return "\n".join(lines)
...
def v301_run_regression_tests():
    ...
    print('V30.1 regression failed: '+str(failed) if failed else 'V30.1 regression OK')
    return not failed

try:
    v301_run_regression_tests()
except Exception as e:
    print('V30.1 regression check kļūda:', repr(e))
...
def init_db():
    ...
    # V30.1 Short Conversation Memory
    db_execute(c, """
    ...
    # V30.1 Memory Intelligence topic statistics
    db_execute(c, """
...
def subscription_info(user_id=None):
    """V30.1: Premium pārdošanas teksts ar cilvēkam saprotamu vērtību."""
...
def premium_conversion_answer(user_id):
    """V30.1: labāks Free -> Premium pārdošanas teksts."""
    user = get_user(user_id)
    if user.get("premium"):
...
def save_natural_memory(user_id, memory_text):
    """V30.1: dabiskās atmiņas saglabāšana pārvietota uz memory.py loģiku."""
    return save_natural_memory_logic(get_db, db_execute, user_id, memory_text)



def latest_natural_memories(user_id, limit=3):
    """V30.1: pēdējo atmiņu nolasīšana pārvietota uz memory.py loģiku."""
    return latest_natural_memories_logic(get_db, db_execute, user_id, limit)



def nina_memory_saved_answer(saved_text):
    """V30.1: atmiņas saglabāšanas teksts no memory.py vai fallback."""
    return build_memory_saved_answer(saved_text, version="V30.1")



def nina_goal_saved_answer(goal_text):
    """V30.1: mērķa saglabāšanas teksts no memory.py vai fallback."""
    return build_goal_saved_answer(goal_text, version="V30.1")



def nina_daily_habit_answer(user_id):
    """V30.1: Daily Assistant ar coach.py + brain.py secinājumiem."""
    try:
        user = get_user(str(user_id))
    except Exception as e:
...
def nina_morning_answer(user_id):
    """V30.1: labrīta teksts no daily.py vai fallback."""
    try:
        user = get_user(str(user_id))
        name = (user.get("name") or "").strip()
    except Exception:
        name = ""
    return build_morning_answer(name=name, version="V30.1")



def nina_evening_answer(user_id):
    """V30.1: vakara teksts no daily.py vai fallback."""
    return build_evening_answer(version="V30.1")



def nina_today_goal_answer(user_id):
    """V30.1: mērķa teksta sagatave no daily.py vai fallback."""
    return build_goal_prompt_answer(version="V30.1")
...
# V30.1 natural conversation logic is in conversation.py



def record_memory_topics(user_id, memory_text):
    """V30.1: pēc atmiņas saglabāšanas pieraksta tēmas brain/analytics vajadzībām."""
    try:
...
def user_topic_counts(user_id, limit=5):
    """V30.1: atgriež lietotāja dominējošo tēmu skaitītāju."""
    conn = None
    try:
        conn = get_db()
        c = conn.cursor()
...
def nina_progress_answer(user_id):
    """V30.1: lietotāja progress ar analytics.py + brain.py."""
    try:
        user = get_user(str(user_id))
    except Exception as e:
    ...
    if not memories and not goals and reminders == 0:
        return build_empty_progress_text(version="V30.1")

    return build_weekly_progress_text(snapshot, topic_counts=topic_counts, version="V30.1")



async def safe_reply_text(update, text, disable_web_page_preview=True):
    """V30.1: publiskā testa drošība — nekad neatstāj lietotāju bez atbildes."""
    try:
...
def nina_public_offer_answer(user_text=""):
    """V30.1: jebkuram lietotājam dzīva atbilde un piedāvājums."""
    ...
    return charm_smalltalk_answer(version="V30.1")



def nina_rough_message_answer():
    return charm_rough_answer(version="V30.1")
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """V30.1: Telegram foto apstrāde ar Vision Engine."""
    try:
        user_id = str(update.effective_user.id)
...
async def reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # V30.1 public reply wrapper
    try:
        user_text = update.message.text
        user_id = str(update.effective_user.id)
        lower = user_text.strip().lower()

        # V30.1 Stable Priority Router: conversation first, profile second.
        ...
        if v301_answer:
            await safe_reply_text(update, v301_answer)
            return
        ...
        if v301_fact.get("type") and v301_fact.get("value"):
            try:
                ...
                print("V30.1 profile save kļūda:", repr(e))
            await safe_reply_text(update, v301_profile_saved_answer(user_id, v301_fact["type"], v301_fact["value"]))
            return


        # V30.1: User Profile DB route.
        if any(x in lower for x in ["ko tu par mani atceries", "ko atceries par mani", "mans profils", "profils"]):
            await safe_reply_text(update, v24_profile_recall_answer(user_id))
            return
        ...
        # V30.1: Smart Sales price/tariff route.
        if v20_is_price_question(user_text):
            await safe_reply_text(update, v20_smart_price_answer(user_id))
            return


        # V30.1: HARD priority capabilities route before old dialog/charm.
        ...
        # V30.1: Human Mode priority route. Tam jābūt pirms vecā dialog.py.
        if v18_should_use_human_mode(user_text):
            await safe_reply_text(update, v19_human_mode_answer_with_memory(user_id, user_text))
            return


        # V30.1: Living Conversation Core priority route.
        if v18_should_use_human_mode(user_text):
            await safe_reply_text(update, v19_human_mode_answer_with_memory(user_id, user_text))
            return


        # V30.1: Dialog smart route. Jautājumi nav atmiņas.
        dialog_kind = classify_dialog_message(user_text)
        if dialog_kind == "capabilities" or dialog_kind == "question":
            await safe_reply_text(update, build_capabilities_answer(version="V30.1"))
            return

        if dialog_kind == "rough_playful":
            await safe_reply_text(update, build_playful_rough_answer(version="V30.1"))
            return

        if dialog_kind == "smalltalk":
            await safe_reply_text(update, build_smalltalk_answer(user_text, version="V30.1"))
            return


        # V30.1: Always Reply public safety.
        if looks_like_rough_message(user_text):
            await safe_reply_text(update, nina_rough_message_answer())
            return


        # V30.1: Progress command.
        ...
        # V30.1: Reminder command.
        reminder_data = parse_reminder_request(user_text, DEFAULT_TIMEZONE)
        if reminder_data is not None:
            if not reminder_data.get("ok"):
                await update.message.reply_text(build_reminder_help_answer(version="V30.1"), disable_web_page_preview=True)
                return
        ...
        # V30.1: Mana diena top-priority route.
        ...
        # V30.1: Natural memory and daily goal capture — immediate replies.
        ...
        # V30.1: Natural conversation auto-handle.
        natural_kind = classify_natural_message(user_text)

        if natural_kind == "goal":
            goal_text = user_text.strip()
            ok = save_daily_goal(user_id, goal_text)
            answer = build_auto_goal_answer(goal_text, version="V30.1") if ok else "Neizdevās saglabāt mērķi. Pamēģini vēlreiz."
            await update.message.reply_text(answer, disable_web_page_preview=True)
            return
        ...
        if natural_kind == "memory":
            ...
            if saved:
                record_memory_topics(user_id, saved)
            answer = build_auto_memory_answer(saved or memory_text, version="V30.1") if saved else "Neizdevās saglabāt. Pamēģini vēlreiz."
            await update.message.reply_text(answer, disable_web_page_preview=True)
            return
    ...
    """
        ...
        # V30.1: Final catch-all so Nina never stays silent.
...
@app.route("/")
def home():
    return "Nina7727 V30.1 Premium Sales Text darbojas! DB: " + ("PostgreSQL" if USE_POSTGRES else "SQLite fallback")
...
if __name__ == "__main__":
    print("Nina7727 V30.1 Premium Sales Text darbojas...", "PostgreSQL" if USE_POSTGRES else "SQLite fallback")
