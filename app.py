...
def stripe_webhook():
    ...
    session = event.data.object if not isinstance(event, dict) else event.get("data", {}).get("object", {}) or {}

    metadata = (session.metadata if hasattr(session, "metadata") else session.get("metadata", {})) or {}
    user_id = (
...
@app.route("/")
def home():
    return "Nina7727 V12.5.2 Stripe Webhook Event Fix darbojas! DB: " + ("PostgreSQL" if USE_POSTGRES else "SQLite fallback")
...
if __name__ == "__main__":
    print("Nina7727 V12.5.2 Stripe Webhook Event Fix darbojas...", "PostgreSQL" if USE_POSTGRES else "SQLite fallback")
