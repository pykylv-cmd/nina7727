...
def subscription_info(user_id=None):
    """V13.1.1: Premium pārdošanas teksts ar cilvēkam saprotamu vērtību."""
...
def premium_conversion_answer(user_id):
    """V13.1.1: labāks Free -> Premium pārdošanas teksts."""
    user = get_user(user_id)
    if user.get("premium"):
...
def nina_daily_habit_answer(user_id):
    """V13.1.1: Daily Assistant — 'mana diena' kā īsta asistente, nevis statusa saraksts."""
    user = get_user(str(user_id))
    ...
    if reminders == 0:
        reminder_text = "Šobrīd tev nav aktīvu atgādinājumu. Tas nozīmē, ka varam mierīgi saplānot, kas šodien ir svarīgākais."
    else:
        reminder_text = f"Tev ir {reminders} aktīvi atgādinājumi. Es palīdzēšu nepalaist garām svarīgo."

    if backups == 0:
        memory_text = "Es vēl neatceros nevienu svarīgu lietu, ko esi man uzticējis. Vari sākt ar vienu mazu lietu."
    else:
    ...
    return (
        ...
        f"{premium_line}\n"
        f"Plāns: {plan}\n\n"
        "Šodien iesaku sākt ar vienu jautājumu:\n"
        "Kas šodien ir pats svarīgākais, ko vajag izdarīt?\n\n"
        "Vari man rakstīt vienkārši:\n"
        "• šodien jāizdara...\n"
        "• atceries, ka...\n"
        "• vakarā atgādini man...\n\n"
        "Es palīdzēšu sakārtot domas, atcerēties svarīgo un virzīties uz priekšu.\n\n"
        "Versija: V13.1.1"
    )


def nina_morning_answer(user_id):
    """V13.1.1: labrīta asistente."""
    user = get_user(str(user_id))
    name = (user.get("name") or "").strip()
...
def nina_evening_answer(user_id):
    """V13.1.1: vakara kopsavilkuma sagatave."""
    return (
...
async def reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # V13.1.1: Daily Assistant extra commands.
...
@app.route("/")
def home():
    return "Nina7727 V13.1.1 Premium Sales Text darbojas! DB: " + ("PostgreSQL" if USE_POSTGRES else "SQLite fallback")
...
if __name__ == "__main__":
    print("Nina7727 V13.1.1 Premium Sales Text darbojas...", "PostgreSQL" if USE_POSTGRES else "SQLite fallback")
