...
# V70.0 Safe User Profile Engine Import
try:
    ...
    def build_profile_saved_answer(fact_type, value, version="V70.0"):
        if fact_type == "name":
...
# V70.0 Safe Vision Engine Import
try:
    ...
    def build_vision_answer_from_openai(client, image_bytes, caption="", version="V70.0"):
        return build_no_vision_fallback(version=version)

    def build_no_vision_fallback(version="V70.0"):
        return (
...
# V70.0 Safe Daily Module Import
...
try:
    ...
    def build_daily_answer(name="", plan="Free", is_premium=False, goals=None, memories=None, reminders=0, version="V70.0"):
        goals = goals or []
    ...
    def build_morning_answer(name="", version="V70.0"):
    ...
    def build_evening_answer(version="V70.0"):
        return (
    ...
    def build_goal_prompt_answer(version="V70.0"):
        return (
            "🎯 Šodienas mērķis\n\n"
...
# V70.0 Safe Memory Module Import
...
try:
    ...
    def build_memory_saved_answer(saved_text, version="V70.0"):
        return (
            "🧠 Pierakstīju. ✅\n\n"
    ...
    def build_goal_saved_answer(goal_text, version="V70.0"):
        return (
...
# V70.0 Safe Conversation Module Import
...
try:
    ...
    def build_auto_memory_answer(memory_text, version="V70.0"):
        return (
            "🧠 Saglabāju. ✅\n\n"
    ...
    def build_auto_goal_answer(goal_text, version="V70.0"):
        return (
...
# V70.0 Safe Coach Module Import
try:
...
# V70.0 Safe Personality Module Import
try:
...
# V70.0 Safe Reminders Module Import
...
try:
    ...
    def build_reminder_saved_answer(reminder_text, human_time="", version="V70.0"):
        if human_time:
            return (
    ...
    def build_reminder_help_answer(version="V70.0"):
        return (
            "⏰ Raksti šādi:\n\n"
...
# V70.0 Safe Brain Module Import
try:
...
# V70.0 Safe Analytics Module Import
...
try:
    ...
    def build_weekly_progress_text(snapshot, topic_counts=None, version="V70.0"):
        return (
    ...
    def build_empty_progress_text(version="V70.0"):
        return (
            "📊 Tavs progress ar Ninu\n\n"
...
# V70.0 Safe Dialog Module Import
...
try:
    ...
    def build_capabilities_answer(version="V70.0"):
        return (
    ...
    def build_playful_rough_answer(version="V70.0"):
        return (
    ...
    def build_smalltalk_answer(user_text="", version="V70.0"):
        return (
            "Esmu te. 😊\n\n"
...
# V70.0 Safe Charm Module Import
try:
    from charm import (
    ...
    def charm_capabilities_answer(version="V70.0"):
        return (
    ...
    def charm_smalltalk_answer(user_text="", version="V70.0"):
        return (
            "Čau. 😊 Esmu te.\n\n"
    ...
    def charm_rough_answer(version="V70.0"):
        return (
...
# V70.0 Safe Persona Engine Import
try:
...
# V70.0 Living Conversation Core
...
def v18_should_use_human_mode(text):
    """V70.0: nosaka, kad Nina runā kā cilvēks, nevis ar vecām robota atbildēm."""
    lower = (text or "").strip().lower()
    if not lower:
        return True
...
def v18_human_mode_answer(text):
    """V70.0 Human Mode: fokusējas uz cilvēku, emociju un nākamo ziņu."""
    lower = (text or "").strip().lower()
...
def v18_human_capabilities_answer():
    """V70.0: īpaši cilvēcisks teksts jautājumam 'ko vari darīt?'."""
    try:
        import random
        variants = [
            (
...
def save_conversation_state(user_id, user_text, nina_text="", intent="", emotion="", topic=""):
    """V70.0: saglabā īstermiņa sarunas kontekstu."""
    try:
        conn = get_db()
        c = conn.cursor()
        db_execute(
...
def latest_conversation_state(user_id, limit=3):
    """V70.0: nolasa pēdējo sarunas kontekstu."""
    try:
        conn = get_db()
        c = conn.cursor()
        db_execute(
...
def v19_human_mode_answer_with_memory(user_id, text):
    """V70.0: Human Mode + īstermiņa sarunas atmiņa."""
    lower = (text or "").strip().lower()
    emotion = detect_emotion(text)
    topic = detect_topic(text)
...
def v20_smart_price_answer(user_id=None):
    """V70.0: atbild uz cenu/tarifu jautājumiem cilvēciski un komerciāli."""
    try:
...
def v21_memory_answer(memory_text,version="V70.0"):
    return (
        "🧠 Saglabāju. ✅\n\n"
        f"Atcerēšos: {memory_text}\n\n"
        "💬 Vai gribi, lai es tev par to arī atgādinu īstajā laikā?\n"
...
def v21_build_memory_answer(memory_text, version="V70.0"):
    memory_text = (memory_text or "").strip()
...
def v24_save_profile_fact_to_db(user_id, fact_type, value):
    """V70.0: saglabā profila faktu users tabulā, lai Nina to atceras pēc restartēšanas."""
...
def v24_profile_answer_from_fact(user_id, user_text):
    """V70.0: atpazīst profila faktus un uzreiz saglabā datubāzē."""
    ...
    return build_profile_saved_answer(fact_type, value, version="V70.0")


def v24_profile_recall_answer(user_id):
    """V70.0: parāda, ko Nina jau zina par lietotāju."""
    user = get_user(str(user_id))
...
def v301_capabilities(user_id):
    ...
    lines += ["", "Tev nav jāiegaumē komandas. Raksti normāli, piemēram:", "rīt jāzvana klientam", "man šodien daudz darbu", "apskati šo bildi", "", "Versija: V70.0"]
    return "\n".join(lines)
...
def v301_run_regression_tests():
    ...
    print('V70.0 regression failed: '+str(failed) if failed else 'V70.0 regression OK')
    return not failed

try:
    v301_run_regression_tests()
except Exception as e:
    print('V70.0 regression check kļūda:', repr(e))


# =========================
# V70.0 REVENUE PLATFORM RELEASE
...
def v401_regression_check():
    ...
    failed = [k for k, ok in tests.items() if not ok]
    if failed:
        print("V70.0 regression failed:", failed)
    else:
        print("V70.0 regression OK")
    return not failed
...
# =========================
# V70.0 AI ASSISTANT PLATFORM RELEASE
...
# =========================
# V70.0 INTELLIGENCE + NAVIGATION RELEASE
...
def v601_navigation_answer(user_id, text):
    ...
    if any(x in lower for x in ["mājās", "majas", "uz mājām", "uz majam"]):
        try:
            return v60_navigation_answer(user_id, text).replace("V70.0", "V70.0")
        except Exception:
            pass
...
def v601_intent_router(user_id, text):
    ...
    try:
        if v60_is_urgent_help(text):
            return v60_urgent_help_answer(user_id, text).replace("V70.0", "V70.0")
    except Exception:
        pass
...
def v602_navigation_answer(user_id, text):
    ...
    try:
        return v601_navigation_answer(user_id, text).replace("V70.0", "V70.0")
    except Exception:
        try:
            return v60_navigation_answer(user_id, text).replace("V70.0", "V70.0").replace("V70.0", "V70.0")
        except Exception:
            return (
...
def v602_intent_router(user_id, text):
    try:
        if v60_is_urgent_help(text):
            return v60_urgent_help_answer(user_id, text).replace("V70.0", "V70.0").replace("V70.0", "V70.0")
    except Exception:
        pass
...
# =========================
# V70.0 BRAIN UPGRADE
# Personality + Intent + Relationship
# =========================

def v70_pick(user_id, intent, variants):
    try:
        import random
...
def v70_ctx(user_id):
    ...
    return {
        ...
        "projects": v70_items(user.get("projects", "")),
        "interests": v70_items(user.get("hobbies", "")),
    }


def v70_is_business(text):
    lower = (text or "").strip().lower()
    return any(x in lower for x in [
        "vajag klientus", "gribu klientus", "vairāk klientu", "vairak klientu",
        "gribu pelnīt", "gribu pelnit", "gribu nopelnīt", "gribu nopelnit",
        "kā pelnīt", "ka pelnit", "kur dabūt klientus", "kur dabut klientus",
        "kā pārdot", "ka pardot", "bizness", "piedāvājums", "piedavajums"
    ])


def v70_business_answer(user_id, text):
    c = v70_ctx(user_id); name = c["name"]; prefix = f"{name}, " if name else ""
    return v70_pick(user_id, "business", [
        f"{prefix}šis jau ir biznesa jautājums. 💼\n\nLai to pārvērstu naudā, vajag skaidru piedāvājumu.\n\nUzraksti man 3 lietas:\n1. Ko tu pārdod vai vari pārdot?\n2. Kam tas visvairāk vajadzīgs?\n3. Kādu cenu gribi prasīt?\n\nTad es palīdzēšu salikt pirmo klientu piesaistes plānu.\n\nVersija: V70.0",
        f"{prefix}labi, ejam praktiski. Nauda sākas no konkrēta piedāvājuma, nevis no domāšanas. 💶\n\nUzraksti: mans pakalpojums ir...\nUn es palīdzēšu to pārvērst tekstā, ko var sūtīt klientam.\n\nVersija: V70.0",
        f"{prefix}ja vajag vairāk klientu, tad taisām mazu sistēmu. 📈\n\nŠodienas minimums:\n• viens piedāvājums;\n• 10 potenciālie klienti;\n• īsa ziņa viņiem.\n\nGribi, lai uzrakstu pirmo ziņu klientiem?\n\nVersija: V70.0",
    ])


def v70_is_emotion(text):
    lower = (text or "").strip().lower()
    return any(x in lower for x in [
        "man slikti", "man smagi", "viss besī", "viss besi", "nav spēka", "nav speka",
        "noguris", "nogurusi", "grūti", "gruti", "panika", "stress", "skumji", "nav labi"
    ])


def v70_emotion_answer(user_id, text):
    c = v70_ctx(user_id); name = c["name"]; prefix = f"{name}, " if name else ""
    return v70_pick(user_id, "emotion", [
        f"{prefix}dzirdēju. Nesteigsimies ar lieliem plāniem. 😔\n\nPasaki vienu lietu: kas šobrīd spiež visvairāk — darbi, cilvēki, nauda vai galvā haoss?\n\nVersija: V70.0",
        f"{prefix}tad ejam mierīgi. Nevajag visu atrisināt uzreiz.\n\nKas ir viens mazs solis, ko vari izdarīt tuvākajās 10 minūtēs?\n\nVersija: V70.0",
        f"{prefix}esmu tepat. Ja ir smagi, sākam nevis ar visu dzīvi, bet ar nākamo soli.\n\nKas notika tieši pirms palika tik smagi?\n\nVersija: V70.0",
    ])


def v70_is_joke(text):
    lower = (text or "").strip().lower()
    return any(x in lower for x in [
        "bla bla", "asdf", "lalala", "es esmu citplanētietis", "esmu citplanetietis",
        "tu esi banāns", "tu esi bananas", "muļķības", "mulkibas", "haha", "lol", "kartupelis"
    ])


def v70_joke_answer(user_id, text):
    return v70_pick(user_id, "joke", [
        "😄 Pieņemts. Kvalitātes tests turpinās. Tagad dod man vienu īstu uzdevumu.\n\nVersija: V70.0",
        "Haha, labi. Tu pārbaudi, vai es atkārtojos. Es pamanīju. 😏 Kas ir īstais jautājums?\n\nVersija: V70.0",
        "Kosmoss pieņemts. 🚀 Tagad nolaižamies uz Zemes: kas šodien jāsakārto?\n\nVersija: V70.0",
        "😄 Es varu jokot līdzi, bet varu arī strādāt. Izvēlies: muļķības vai lieta?\n\nVersija: V70.0",
    ])


def v70_greeting(user_id):
    c = v70_ctx(user_id)
    name, projects, interests, profession = c["name"], c["projects"], c["interests"], c["profession"]
    hello = f"Čau, {name}. 😊" if name else "Čau. 😊"
    variants = []
    if projects:
        variants.append(f"{hello}\n\nAtceros tavu tēmu: {projects[-1]}.\nKā tur virzās — ir kustība vai kaut kas bremzē?\n\nVersija: V70.0")
    if profession:
        variants.append(f"{hello}\n\nKā šodien iet tavā jomā — {profession}? Klienti, termiņi vai plānošana?\n\nVersija: V70.0")
    if interests:
        variants.append(f"{hello}\n\nAtceros, ka tev interesē {interests[-1]}. Ko šodien ar to pavirzām uz priekšu?\n\nVersija: V70.0")
    variants += [
        f"{hello}\n\nKas šodien jāsakārto — darbi, cilvēki, nauda vai haoss galvā?\n\nVersija: V70.0",
        f"{hello}\n\nDod man vienu reālu lietu no šodienas, un es palīdzēšu to salikt pa soļiem.\n\nVersija: V70.0",
    ]
    return v70_pick(user_id, "greeting", variants)


def v70_is_capabilities(text):
    lower = (text or "").strip().lower()
    return any(x in lower for x in ["ko vari", "ko tu vari", "ko māki", "ko maki", "ko vari darīt", "ko vari darit"])


def v70_capabilities(user_id):
    c = v70_ctx(user_id); prefix = f"{c['name']}, " if c["name"] else ""
    return v70_pick(user_id, "capabilities", [
        f"{prefix}es varu palīdzēt ne tikai čatot, bet arī sakārtot praktiskas lietas. 😉\n\n🧠 atmiņas\n⏰ atgādinājumi\n📍 navigācija\n💼 klienti un nauda\n💬 saruna, kad galvā haoss\n\nPasaki vienu īstu situāciju, un es parādīšu.\n\nVersija: V70.0",
        f"{prefix}labāk parādīšu praksē. Uzraksti kaut ko no dzīves:\n• rīt jāzvana klientam\n• vajag nokļūt uz Rīgu\n• gribu vairāk klientu\n• man galvā haoss\n\nVersija: V70.0",
    ])


def v70_intent_router(user_id, text):
    lower = (text or "").strip().lower()
    if lower in ["čau", "cau", "sveika", "sveiks", "hi", "hello", "hei", "labdien"]:
        return v70_greeting(user_id)
    if v70_is_capabilities(text):
        return v70_capabilities(user_id)
    if v70_is_joke(text):
        return v70_joke_answer(user_id, text)
    if v70_is_emotion(text):
        return v70_emotion_answer(user_id, text)
    if v70_is_business(text):
        return v70_business_answer(user_id, text)
    return None


def v70_regression_check():
    tests = {
        "business": v70_is_business("vajag vairāk klientu"),
        "emotion": v70_is_emotion("man smagi"),
        "joke": v70_is_joke("es esmu citplanētietis"),
        "capabilities": v70_is_capabilities("ko vari darīt"),
    }
    failed = [k for k, ok in tests.items() if not ok]
    print("V70 regression failed:" if failed else "V70 regression OK", failed if failed else "")
    return not failed

try:
    v70_regression_check()
except Exception as e:
    print('V70 regression check kļūda:', repr(e))
...
def init_db():
    ...
    # V70.0 Short Conversation Memory
    db_execute(c, """
    ...
    # V70.0 Memory Intelligence topic statistics
    db_execute(c, """
    ...
    # V70.0 Revenue Core: usage events and commercial signals
    db_execute(c, """
    ...
    # V70.0 Assistant Platform tables
    db_execute(c, """
    ...
    # V70.0 Intelligence Layer: locations and conversation quality
    db_execute(c, """
...
def subscription_info(user_id=None):
    """V70.0: Premium pārdošanas teksts ar cilvēkam saprotamu vērtību."""
...
def premium_conversion_answer(user_id):
    """V70.0: labāks Free -> Premium pārdošanas teksts."""
    user = get_user(user_id)
    if user.get("premium"):
...
def save_natural_memory(user_id, memory_text):
    """V70.0: dabiskās atmiņas saglabāšana pārvietota uz memory.py loģiku."""
    return save_natural_memory_logic(get_db, db_execute, user_id, memory_text)



def latest_natural_memories(user_id, limit=3):
    """V70.0: pēdējo atmiņu nolasīšana pārvietota uz memory.py loģiku."""
    return latest_natural_memories_logic(get_db, db_execute, user_id, limit)



def nina_memory_saved_answer(saved_text):
    """V70.0: atmiņas saglabāšanas teksts no memory.py vai fallback."""
    return build_memory_saved_answer(saved_text, version="V70.0")



def nina_goal_saved_answer(goal_text):
    """V70.0: mērķa saglabāšanas teksts no memory.py vai fallback."""
    return build_goal_saved_answer(goal_text, version="V70.0")



def nina_daily_habit_answer(user_id):
    """V70.0: Daily Assistant ar coach.py + brain.py secinājumiem."""
    try:
        user = get_user(str(user_id))
    except Exception as e:
...
def nina_morning_answer(user_id):
    """V70.0: labrīta teksts no daily.py vai fallback."""
    try:
        user = get_user(str(user_id))
        name = (user.get("name") or "").strip()
    except Exception:
        name = ""
    return build_morning_answer(name=name, version="V70.0")



def nina_evening_answer(user_id):
    """V70.0: vakara teksts no daily.py vai fallback."""
    return build_evening_answer(version="V70.0")



def nina_today_goal_answer(user_id):
    """V70.0: mērķa teksta sagatave no daily.py vai fallback."""
    return build_goal_prompt_answer(version="V70.0")
...
# V70.0 natural conversation logic is in conversation.py



def record_memory_topics(user_id, memory_text):
    """V70.0: pēc atmiņas saglabāšanas pieraksta tēmas brain/analytics vajadzībām."""
    try:
...
def user_topic_counts(user_id, limit=5):
    """V70.0: atgriež lietotāja dominējošo tēmu skaitītāju."""
    conn = None
    try:
        conn = get_db()
        c = conn.cursor()
...
def nina_progress_answer(user_id):
    """V70.0: lietotāja progress ar analytics.py + brain.py."""
    try:
        user = get_user(str(user_id))
    except Exception as e:
    ...
    if not memories and not goals and reminders == 0:
        return build_empty_progress_text(version="V70.0")

    return build_weekly_progress_text(snapshot, topic_counts=topic_counts, version="V70.0")



async def safe_reply_text(update, text, disable_web_page_preview=True):
    """V70.0: publiskā testa drošība — nekad neatstāj lietotāju bez atbildes."""
    try:
...
def nina_public_offer_answer(user_text=""):
    """V70.0: jebkuram lietotājam dzīva atbilde un piedāvājums."""
    ...
    return charm_smalltalk_answer(version="V70.0")



def nina_rough_message_answer():
    return charm_rough_answer(version="V70.0")
...
async def handle_location(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """V70.0: Telegram location support."""
    try:
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """V70.0: Telegram foto apstrāde ar Vision Engine."""
    try:
        user_id = str(update.effective_user.id)
...
async def reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # V70.0 public reply wrapper
    try:
        user_text = update.message.text
        user_id = str(update.effective_user.id)
        lower = user_text.strip().lower()

        # V70.0 Brain Upgrade Router.
        v70_answer = v70_intent_router(user_id, user_text)
        if v70_answer:
            try:
                v40_log_usage(user_id, "v70_brain", user_text)
            except Exception:
                pass
            await safe_reply_text(update, v70_answer)
            return


        # V70.0 Navigation Polish Router.
        ...
        # V70.0 Navigation Hotfix Router: emergency/navigation before generic help.
        ...
        # V70.0 Intelligence + Navigation Router.
        ...
        # V70.0 Assistant Platform Router.
        ...
        # V70.0 Stable Intent Router: practical help before profile/revenue.
        ...
        # V70.0 Stable Priority Router: conversation first, profile second.
        ...
        if v301_fact.get("type") and v301_fact.get("value"):
            try:
                ...
                print("V70.0 profile save kļūda:", repr(e))
        ...
        # V70.0: Smart Sales price/tariff route.
        if v20_is_price_question(user_text):
            await safe_reply_text(update, v20_smart_price_answer(user_id))
            return


        # V70.0: HARD priority capabilities route before old dialog/charm.
        ...
        # V70.0: Human Mode priority route. Tam jābūt pirms vecā dialog.py.
        if v18_should_use_human_mode(user_text):
            await safe_reply_text(update, v19_human_mode_answer_with_memory(user_id, user_text))
            return


        # V70.0: Living Conversation Core priority route.
        if v18_should_use_human_mode(user_text):
            await safe_reply_text(update, v19_human_mode_answer_with_memory(user_id, user_text))
            return


        # V70.0: Dialog smart route. Jautājumi nav atmiņas.
        dialog_kind = classify_dialog_message(user_text)
        if dialog_kind == "capabilities" or dialog_kind == "question":
            await safe_reply_text(update, build_capabilities_answer(version="V70.0"))
            return

        if dialog_kind == "rough_playful":
            await safe_reply_text(update, build_playful_rough_answer(version="V70.0"))
            return

        if dialog_kind == "smalltalk":
            await safe_reply_text(update, build_smalltalk_answer(user_text, version="V70.0"))
            return


        # V70.0: Always Reply public safety.
        if looks_like_rough_message(user_text):
            await safe_reply_text(update, nina_rough_message_answer())
            return


        # V70.0: Progress command.
        ...
        # V70.0: Reminder command.
        reminder_data = parse_reminder_request(user_text, DEFAULT_TIMEZONE)
        if reminder_data is not None:
            if not reminder_data.get("ok"):
                await update.message.reply_text(build_reminder_help_answer(version="V70.0"), disable_web_page_preview=True)
                return
        ...
        # V70.0: Mana diena top-priority route.
        ...
        # V70.0: Natural memory and daily goal capture — immediate replies.
        ...
        # V70.0: Natural conversation auto-handle.
        natural_kind = classify_natural_message(user_text)

        if natural_kind == "goal":
            goal_text = user_text.strip()
            ok = save_daily_goal(user_id, goal_text)
            answer = build_auto_goal_answer(goal_text, version="V70.0") if ok else "Neizdevās saglabāt mērķi. Pamēģini vēlreiz."
            await update.message.reply_text(answer, disable_web_page_preview=True)
            return
        ...
        if natural_kind == "memory":
            ...
            if saved:
                record_memory_topics(user_id, saved)
            answer = build_auto_memory_answer(saved or memory_text, version="V70.0") if saved else "Neizdevās saglabāt. Pamēģini vēlreiz."
            await update.message.reply_text(answer, disable_web_page_preview=True)
            return
    ...
    """
        ...
        # V70.0: Final catch-all so Nina never stays silent.
...
@app.route("/")
def home():
    return "Nina7727 V70.0 Premium Sales Text darbojas! DB: " + ("PostgreSQL" if USE_POSTGRES else "SQLite fallback")
...
if __name__ == "__main__":
    print("Nina7727 V70.0 Premium Sales Text darbojas...", "PostgreSQL" if USE_POSTGRES else "SQLite fallback")
