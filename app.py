...
# V14.1 Safe Daily Module Import
...
try:
    ...
    def build_daily_answer(name="", plan="Free", is_premium=False, goals=None, memories=None, reminders=0, version="V14.1"):
        goals = goals or []
    ...
    def build_morning_answer(name="", version="V14.1"):
    ...
    def build_evening_answer(version="V14.1"):
        return (
    ...
    def build_goal_prompt_answer(version="V14.1"):
        return (
            "🎯 Šodienas mērķis\n\n"
...
# V14.1 Safe Memory Module Import
# Ja memory.py vēl nav augšupielādēts, app.py joprojām strādā ar fallback tekstiem.
try:
    from memory import (
...
def subscription_info(user_id=None):
    """V14.1: Premium pārdošanas teksts ar cilvēkam saprotamu vērtību."""
...
def premium_conversion_answer(user_id):
    """V14.1: labāks Free -> Premium pārdošanas teksts."""
    user = get_user(user_id)
    if user.get("premium"):
...
# =========================
# V14.1 NATURAL MEMORY + DAILY GOALS
# =========================

def save_daily_goal(user_id, goal_text):
    """V14.1: Saglabā šodienas mērķi, aizvietojot veco šodienas mērķi."""
...
def latest_daily_goals(user_id, limit=3):
    """V14.1: Atgriež šodienas aktīvo mērķi/mērķus."""
    today = datetime.now(ZoneInfo(DEFAULT_TIMEZONE)).strftime("%Y-%m-%d")
...
def save_natural_memory(user_id, memory_text):
    """Saglabā lietotāja dabisko 'atceries...' frāzi kā memory backup."""
    memory_text = (memory_text or "").strip()
    if not memory_text:
        return False

    # Noņem sākuma frāzes, lai saglabātā doma ir tīrāka.
...
def latest_natural_memories(user_id, limit=3):
    """V14.1: Droši atgriež pēdējās svarīgās lietas."""
    try:
        conn = get_db()
        c = conn.cursor()
        db_execute(
...
def nina_memory_saved_answer(saved_text):
    """V14.1: atmiņas saglabāšanas teksts no memory.py vai fallback."""
    return build_memory_saved_answer(saved_text, version="V14.1")



def nina_goal_saved_answer(goal_text):
    """V14.1: mērķa saglabāšanas teksts no memory.py vai fallback."""
    return build_goal_saved_answer(goal_text, version="V14.1")



def nina_daily_habit_answer(user_id):
    """V14.1: Daily Assistant loģika paliek app.py, teksti no daily.py vai fallback."""
    try:
        user = get_user(str(user_id))
    except Exception as e:
    ...
    try:
        raw_memories = latest_natural_memories(str(user_id), limit=3)
...
def nina_morning_answer(user_id):
    """V14.1: labrīta teksts no daily.py vai fallback."""
    try:
        user = get_user(str(user_id))
        name = (user.get("name") or "").strip()
    except Exception:
        name = ""
    return build_morning_answer(name=name, version="V14.1")



def nina_evening_answer(user_id):
    """V14.1: vakara teksts no daily.py vai fallback."""
    return build_evening_answer(version="V14.1")



def nina_today_goal_answer(user_id):
    """V14.1: mērķa teksta sagatave no daily.py vai fallback."""
    return build_goal_prompt_answer(version="V14.1")
...
async def reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # V14.1: Mana diena top-priority route.
    ...
    # V14.1: Natural memory and daily goal capture — immediate replies.
...
@app.route("/")
def home():
    return "Nina7727 V14.1 Premium Sales Text darbojas! DB: " + ("PostgreSQL" if USE_POSTGRES else "SQLite fallback")
...
if __name__ == "__main__":
    print("Nina7727 V14.1 Premium Sales Text darbojas...", "PostgreSQL" if USE_POSTGRES else "SQLite fallback")
...
# V14.0.1 Safe Daily Module Import
# Ja daily.py vēl nav augšupielādēts, app.py joprojām startē ar iebūvētiem fallback tekstiem.
try:
    from daily import (
        build_daily_answer,
...
def subscription_info(user_id=None):
    """V14.0.1: Premium pārdošanas teksts ar cilvēkam saprotamu vērtību."""
    plan = current_plan_name(user_id) if user_id else PLAN_FREE
    user = get_user(user_id) if user_id else {"premium": 0, "premium_until": ""}
...
def premium_conversion_answer(user_id):
    """V14.0.1: labāks Free -> Premium pārdošanas teksts."""
    user = get_user(user_id)
    if user.get("premium"):
        return subscription_info(user_id)
...
# =========================
# V14.0.1 NATURAL MEMORY + DAILY GOALS
# =========================

def save_daily_goal(user_id, goal_text):
    """V14.0.1: Saglabā šodienas mērķi, aizvietojot veco šodienas mērķi."""
    goal_text = (goal_text or "").strip()
    if not goal_text:
        return False
...
def latest_daily_goals(user_id, limit=3):
    """V14.0.1: Atgriež šodienas aktīvo mērķi/mērķus."""
    today = datetime.now(ZoneInfo(DEFAULT_TIMEZONE)).strftime("%Y-%m-%d")
...
def latest_natural_memories(user_id, limit=3):
    """V14.0.1: Droši atgriež pēdējās svarīgās lietas."""
    try:
        conn = get_db()
        c = conn.cursor()
        db_execute(
...
def nina_daily_habit_answer(user_id):
    """V14.0.1: Daily Assistant loģika paliek app.py, teksti no daily.py vai fallback."""
    try:
        user = get_user(str(user_id))
    except Exception as e:
        print("mana diena get_user kļūda:", e)
...
def nina_morning_answer(user_id):
    """V14.0.1: labrīta teksts no daily.py vai fallback."""
    try:
        user = get_user(str(user_id))
        name = (user.get("name") or "").strip()
    except Exception:
        name = ""
    return build_morning_answer(name=name, version="V14.0.1")



def nina_evening_answer(user_id):
    """V14.0.1: vakara teksts no daily.py vai fallback."""
    return build_evening_answer(version="V14.0.1")



def nina_today_goal_answer(user_id):
    """V14.0.1: mērķa teksta sagatave no daily.py vai fallback."""
    return build_goal_prompt_answer(version="V14.0.1")
...
async def reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # V14.0.1: Mana diena top-priority route.
    if lower in ["mana diena", "diena", "my day"]:
        await update.message.reply_text(
            nina_daily_habit_answer(user_id),
            disable_web_page_preview=True
    ...
    # V14.0.1: Natural memory and daily goal capture — immediate replies.
    if lower.startswith("atceries,") or lower.startswith("atceries ka") or lower.startswith("atceries "):
        saved = save_natural_memory(user_id, user_text)
        if saved:
            answer = nina_memory_saved_answer(saved)
...
@app.route("/")
def home():
    return "Nina7727 V14.0.1 Premium Sales Text darbojas! DB: " + ("PostgreSQL" if USE_POSTGRES else "SQLite fallback")
...
if __name__ == "__main__":
    print("Nina7727 V14.0.1 Premium Sales Text darbojas...", "PostgreSQL" if USE_POSTGRES else "SQLite fallback")
