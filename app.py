...
# V30.0 Safe User Profile Engine Import
try:
    ...
    def build_profile_saved_answer(fact_type, value, version="V30.0"):
        if fact_type == "name":
...
# V30.0 Safe Vision Engine Import
try:
    ...
    def build_vision_answer_from_openai(client, image_bytes, caption="", version="V30.0"):
        return build_no_vision_fallback(version=version)

    def build_no_vision_fallback(version="V30.0"):
        return (
...
# V30.0 Safe Daily Module Import
...
try:
    ...
    def build_daily_answer(name="", plan="Free", is_premium=False, goals=None, memories=None, reminders=0, version="V30.0"):
        goals = goals or []
    ...
    def build_morning_answer(name="", version="V30.0"):
    ...
    def build_evening_answer(version="V30.0"):
        return (
    ...
    def build_goal_prompt_answer(version="V30.0"):
        return (
            "🎯 Šodienas mērķis\n\n"
...
# V30.0 Safe Memory Module Import
...
try:
    ...
    def build_memory_saved_answer(saved_text, version="V30.0"):
        return (
            "🧠 Pierakstīju. ✅\n\n"
    ...
    def build_goal_saved_answer(goal_text, version="V30.0"):
        return (
...
# V30.0 Safe Conversation Module Import
...
try:
    ...
    def build_auto_memory_answer(memory_text, version="V30.0"):
        return (
            "🧠 Saglabāju. ✅\n\n"
    ...
    def build_auto_goal_answer(goal_text, version="V30.0"):
        return (
...
# V30.0 Safe Coach Module Import
try:
...
# V30.0 Safe Personality Module Import
try:
...
# V30.0 Safe Reminders Module Import
...
try:
    ...
    def build_reminder_saved_answer(reminder_text, human_time="", version="V30.0"):
        if human_time:
            return (
    ...
    def build_reminder_help_answer(version="V30.0"):
        return (
            "⏰ Raksti šādi:\n\n"
...
# V30.0 Safe Brain Module Import
try:
...
# V30.0 Safe Analytics Module Import
...
try:
    ...
    def build_weekly_progress_text(snapshot, topic_counts=None, version="V30.0"):
        return (
    ...
    def build_empty_progress_text(version="V30.0"):
        return (
            "📊 Tavs progress ar Ninu\n\n"
...
# V30.0 Safe Dialog Module Import
...
try:
    ...
    def build_capabilities_answer(version="V30.0"):
        return (
    ...
    def build_playful_rough_answer(version="V30.0"):
        return (
    ...
    def build_smalltalk_answer(user_text="", version="V30.0"):
        return (
            "Esmu te. 😊\n\n"
...
# V30.0 Safe Charm Module Import
try:
    from charm import (
    ...
    def charm_capabilities_answer(version="V30.0"):
        return (
    ...
    def charm_smalltalk_answer(user_text="", version="V30.0"):
        return (
            "Čau. 😊 Esmu te.\n\n"
    ...
    def charm_rough_answer(version="V30.0"):
        return (
...
# V30.0 Safe Persona Engine Import
try:
...
# V30.0 Living Conversation Core
...
def v18_should_use_human_mode(text):
    """V30.0: nosaka, kad Nina runā kā cilvēks, nevis ar vecām robota atbildēm."""
    lower = (text or "").strip().lower()
    if not lower:
        return True
...
def v18_human_mode_answer(text):
    """V30.0 Human Mode: fokusējas uz cilvēku, emociju un nākamo ziņu."""
    lower = (text or "").strip().lower()
...
def v18_human_capabilities_answer():
    """V30.0: īpaši cilvēcisks teksts jautājumam 'ko vari darīt?'."""
    try:
        import random
        variants = [
            (
...
def save_conversation_state(user_id, user_text, nina_text="", intent="", emotion="", topic=""):
    """V30.0: saglabā īstermiņa sarunas kontekstu."""
    try:
        conn = get_db()
        c = conn.cursor()
        db_execute(
...
def latest_conversation_state(user_id, limit=3):
    """V30.0: nolasa pēdējo sarunas kontekstu."""
    try:
        conn = get_db()
        c = conn.cursor()
        db_execute(
...
def v19_human_mode_answer_with_memory(user_id, text):
    """V30.0: Human Mode + īstermiņa sarunas atmiņa."""
    lower = (text or "").strip().lower()
    emotion = detect_emotion(text)
    topic = detect_topic(text)
...
def v20_smart_price_answer(user_id=None):
    """V30.0: atbild uz cenu/tarifu jautājumiem cilvēciski un komerciāli."""
    try:
...
def v21_memory_answer(memory_text,version="V30.0"):
    return (
        "🧠 Saglabāju. ✅\n\n"
        f"Atcerēšos: {memory_text}\n\n"
        "💬 Vai gribi, lai es tev par to arī atgādinu īstajā laikā?\n"
...
def v21_build_memory_answer(memory_text, version="V30.0"):
    memory_text = (memory_text or "").strip()
...
def v24_save_profile_fact_to_db(user_id, fact_type, value):
    """V30.0: saglabā profila faktu users tabulā, lai Nina to atceras pēc restartēšanas."""
...
def v24_profile_answer_from_fact(user_id, user_text):
    """V30.0: atpazīst profila faktus un uzreiz saglabā datubāzē."""
    ...
    return build_profile_saved_answer(fact_type, value, version="V30.0")


def v24_profile_recall_answer(user_id):
    """V30.0: parāda, ko Nina jau zina par lietotāju."""
    user = get_user(str(user_id))
...
# =========================
# V30.0 PLATFORM RELEASE
# Relationship + Profile + Follow-up + Vision 2.0
# =========================

def v30_split_items(value):
    value = (value or "").strip()
    if not value:
        return []
...
def v30_user_display_name(user):
    return ((user or {}).get("name") or "").strip()


def v30_detect_extended_profile_fact(text):
    raw = (text or "").strip()
    lower = raw.lower()
    if not lower:
        return {"type": "", "value": ""}
    ...
    for marker in ["man patīk ", "man patik ", "mani interesē ", "mani interese ", "interesējos par ", "interesejos par "]:
        if marker in lower:
            ...
            if value:
                return {"type": "interest", "value": value[:120]}

    for marker in ["strādāju pie ", "stradaju pie ", "mans projekts ir ", "taisām ", "taisam ", "būvēju ", "buveju "]:
        if marker in lower:
            idx = lower.find(marker)
            value = raw[idx + len(marker):].strip(" .,!?:;")
            if value:
                return {"type": "project", "value": value[:160]}

    if any(w in lower for w in ["sieva", "vīrs", "virs", "bērns", "berns", "ģimene", "gimene"]):
        return {"type": "family", "value": raw[:160]}

    if any(w in lower for w in ["klients", "klienti", "pasūtītājs", "pasutitajs", "darījums", "darijums"]):
        return {"type": "client_topic", "value": raw[:160]}

    return {"type": "", "value": ""}


def v30_save_profile_fact(user_id, fact_type, value):
    value = (value or "").strip()
    if not value:
        return False
    ...
    if fact_type == "name":
        ...
        user["family"] = v24_append_unique_text(user.get("family", ""), value)
    elif fact_type == "client_topic":
        user["facts"] = v24_append_unique_text(user.get("facts", ""), value)
    else:
        user["facts"] = v24_append_unique_text(user.get("facts", ""), value)
...
def v30_profile_saved_answer(user_id, fact_type, value):
    user = get_user(str(user_id))
    name = v30_user_display_name(user)
    prefix = f"{name}, " if name and fact_type != "name" else ""
    ...
    if fact_type == "interest":
        return (
            f"{prefix}noķēru interesi. 🙂\n\n"
            f"Tev svarīga tēma: {value}.\n\n"
            "Tas man palīdzēs runāt precīzāk, nevis kā vispārīgam robotam.\n\n"
            "Versija: V30.0"
        )

    if fact_type == "project":
        return (
            f"{prefix}piefiksēju šo kā projekta tēmu. 🧠\n\n"
            f"{value}\n\n"
    ...
    if fact_type == "client_topic":
        return (
            f"{prefix}saprotu, te ir klientu tēma. 🤝\n\n"
            "Tādas lietas bieži ir vērts pārvērst konkrētā plānā vai atgādinājumā.\n\n"
            "Gribi, lai palīdzēšu to noformulēt kā nākamo soli?\n\n"
            "Versija: V30.0"
        )
...
def v30_relationship_greeting(user_id):
    user = get_user(str(user_id))
    name = v30_user_display_name(user)
    prof = (user.get("profession") or "").strip()
    projects = v30_split_items(user.get("projects", ""))
    interests = v30_split_items(user.get("hobbies", ""))
    ...
    if prof:
        return (
            f"{hello}\n\n"
            f"Kā šodien iet ar tavu jomu — {prof}?\n\n"
            "Kas šobrīd ir svarīgākais: darbi, klienti vai plānošana?\n\n"
            "Versija: V30.0"
        )
    ...
    if interests:
        return (
            f"{hello}\n\n"
            f"Atceros, ka tev interesē {interests[-1]}. Ko šodien gribi ar to pavirzīt uz priekšu?\n\n"
            "Versija: V30.0"
        )
...
def v30_relationship_capabilities(user_id):
    user = get_user(str(user_id))
    name = v30_user_display_name(user)
    prof = (user.get("profession") or "").strip()
    projects = v30_split_items(user.get("projects", ""))
    ...
    start = f"{name}, es varu palīdzēt praktiski, ne tikai čatot. 😉" if name else "Es varu palīdzēt praktiski, ne tikai čatot. 😉"
    ...
    lines = [
        ...
        "⏰ palīdzēt izveidot atgādinājumus;",
        "🎯 sakārtot dienas mērķi;",
        "📷 saprast bildes un dokumentu foto;",
        "💬 palīdzēt izrunāt situāciju, ja galvā haoss;",
    ]

    if prof:
        lines.append(f"💼 pielāgoties tavai jomai: {prof};")
    if projects:
        lines.append(f"🚀 turpināt sarunu par projektu: {projects[-1]};")
    if interests:
        lines.append(f"✨ ņemt vērā tavas intereses: {', '.join(interests[:2])};")

    lines.extend([
        "",
        "Tev nav jāiegaumē komandas. Raksti normāli, piemēram:",
        "rīt jāzvana klientam",
        "man šodien daudz darbu",
        "apskati šo bildi",
        "",
        "Versija: V30.0"
    ])
...
def v30_relationship_daily_question(user_id):
    user = get_user(str(user_id))
    name = v30_user_display_name(user)
    projects = v30_split_items(user.get("projects", ""))
    prof = (user.get("profession") or "").strip()
    ...
    if projects:
        return (
            f"{hello}šodien es ieteiktu sākt ar vienu soli pie: {projects[-1]}.\n\n"
    ...
    if prof:
        return (
            f"{hello}ņemot vērā tavu jomu ({prof}), šodien labāk izvēlēties vienu praktisku prioritāti.\n\n"
            "Kas ir svarīgāk: klients, termiņš vai dokumenti?\n\n"
            "Versija: V30.0"
        )

    return (
        f"{hello}izvēlamies vienu galveno lietu šodienai.\n\n"
        "Kas būtu jāizdara, lai vakarā justos, ka diena nav pagājusi tukši?\n\n"
        "Versija: V30.0"
    )


def v30_followup_answer(user_id, text):
    lower = (text or "").strip().lower()
    user = get_user(str(user_id))
    name = v30_user_display_name(user)
    projects = v30_split_items(user.get("projects", ""))
    ...
    if any(x in lower for x in ["kā tev iet", "ka tev iet", "kā iet", "ka iet"]):
        prefix = f"Man viss labi, {name}. 😊" if name else "Man viss labi. 😊"
        if projects:
            return (
                f"{prefix}\n\n"
                f"Bet svarīgāk — kā iet ar šo: {projects[-1]}?\n\n"
                "Vai šodien vajag palīdzēt to pavirzīt uz priekšu?\n\n"
                "Versija: V30.0"
            )
        return (
            f"{prefix}\n\n"
            "Bet man interesē, kā iet tev. Kas šodien vairāk spiež — darbi, cilvēki vai vienkārši haoss galvā?\n\n"
            "Versija: V30.0"
        )

    if any(x in lower for x in ["ko vari", "ko tu vari", "ko māki", "ko maki"]):
        return v30_relationship_capabilities(user_id)

    if lower in ["čau", "cau", "sveika", "sveiks", "hi", "hello", "hei", "labdien"]:
        return v30_relationship_greeting(user_id)

    if any(x in lower for x in ["ko man darīt", "ko man darit", "ar ko sākt", "ar ko sakt", "ko šodien"]):
        return v30_relationship_daily_question(user_id)

    if any(x in lower for x in ["jā", "ja", "ok", "labi", "davai", "aiziet"]) and projects:
        return (
            f"Labi. Tad ejam pa vienam. 😊\n\n"
            f"Par projektu: {projects[-1]}\n\n"
            "Uzraksti vienu konkrētu lietu, kas tur šobrīd jāizdara. Es palīdzēšu to pārvērst par plānu vai atgādinājumu.\n\n"
            "Versija: V30.0"
        )
    ...
    if any(x in lower for x in ["man smagi", "smagi", "grūti", "gruti", "slikta diena", "noguris", "nogurusi"]):
        ...
        return (
            f"{who}izklausās, ka šobrīd nav viegli. 😔\n\n"
...
def v30_should_offer_soft_premium(user_id):
    try:
        user = get_user(str(user_id))
        if user.get("premium"):
            return False

        memories = latest_natural_memories_logic(get_db, db_execute, str(user_id), limit=10) or []
        reminders = active_reminders_count(str(user_id)) if "active_reminders_count" in globals() else 0
        return len(memories) >= 4 or int(reminders or 0) >= 3
    except Exception:
        return False


def v30_soft_premium_line(user_id):
    if not v30_should_offer_soft_premium(user_id):
        return ""
    return (
        "\n\n💎 Starp citu — tu jau sāc mani izmantot kā īstu palīgu. "
        "Ja gribēsi vairāk atmiņas un atgādinājumu, Premium Basic vēlāk varētu būt loģisks nākamais solis. "
        "Nesteidzies, vispirms izmēģini mani praksē."
    )


def v30_enhance_vision_answer(user_id, answer, caption=""):
    user = get_user(str(user_id))
    name = v30_user_display_name(user)
    prof = (user.get("profession") or "").strip()
    ...
    extra = []
    if name:
        extra.append(f"{name}, es bildi apskatījos praktiski, ne tikai formāli.")
    else:
        extra.append("Bildīti apskatījos praktiski, ne tikai formāli.")

    if prof and any(x in prof.lower() for x in ["celtn", "būv", "buv", "fasād", "fasad"]):
        extra.append("Ja šī bilde ir saistīta ar darbu vai objektu, vari atsūtīt vēl tuvplānu — mēģināšu palīdzēt saprast detaļas.")

    extra.append("Ko tu gribi, lai es ar šo bildi izdaru: vienkārši aprakstu, novērtēju situāciju vai palīdzu pieņemt lēmumu?")
...
def init_db():
    ...
    # V30.0 Short Conversation Memory
    db_execute(c, """
    ...
    # V30.0 Memory Intelligence topic statistics
    db_execute(c, """
...
def subscription_info(user_id=None):
    """V30.0: Premium pārdošanas teksts ar cilvēkam saprotamu vērtību."""
...
def premium_conversion_answer(user_id):
    """V30.0: labāks Free -> Premium pārdošanas teksts."""
    user = get_user(user_id)
    if user.get("premium"):
...
def save_natural_memory(user_id, memory_text):
    """V30.0: dabiskās atmiņas saglabāšana pārvietota uz memory.py loģiku."""
    return save_natural_memory_logic(get_db, db_execute, user_id, memory_text)



def latest_natural_memories(user_id, limit=3):
    """V30.0: pēdējo atmiņu nolasīšana pārvietota uz memory.py loģiku."""
    return latest_natural_memories_logic(get_db, db_execute, user_id, limit)



def nina_memory_saved_answer(saved_text):
    """V30.0: atmiņas saglabāšanas teksts no memory.py vai fallback."""
    return build_memory_saved_answer(saved_text, version="V30.0")



def nina_goal_saved_answer(goal_text):
    """V30.0: mērķa saglabāšanas teksts no memory.py vai fallback."""
    return build_goal_saved_answer(goal_text, version="V30.0")



def nina_daily_habit_answer(user_id):
    """V30.0: Daily Assistant ar coach.py + brain.py secinājumiem."""
    try:
        user = get_user(str(user_id))
    except Exception as e:
...
def nina_morning_answer(user_id):
    """V30.0: labrīta teksts no daily.py vai fallback."""
    try:
        user = get_user(str(user_id))
        name = (user.get("name") or "").strip()
    except Exception:
        name = ""
    return build_morning_answer(name=name, version="V30.0")



def nina_evening_answer(user_id):
    """V30.0: vakara teksts no daily.py vai fallback."""
    return build_evening_answer(version="V30.0")



def nina_today_goal_answer(user_id):
    """V30.0: mērķa teksta sagatave no daily.py vai fallback."""
    return build_goal_prompt_answer(version="V30.0")
...
# V30.0 natural conversation logic is in conversation.py



def record_memory_topics(user_id, memory_text):
    """V30.0: pēc atmiņas saglabāšanas pieraksta tēmas brain/analytics vajadzībām."""
    try:
...
def user_topic_counts(user_id, limit=5):
    """V30.0: atgriež lietotāja dominējošo tēmu skaitītāju."""
    conn = None
    try:
        conn = get_db()
        c = conn.cursor()
...
def nina_progress_answer(user_id):
    """V30.0: lietotāja progress ar analytics.py + brain.py."""
    try:
        user = get_user(str(user_id))
    except Exception as e:
    ...
    if not memories and not goals and reminders == 0:
        return build_empty_progress_text(version="V30.0")

    return build_weekly_progress_text(snapshot, topic_counts=topic_counts, version="V30.0")



async def safe_reply_text(update, text, disable_web_page_preview=True):
    """V30.0: publiskā testa drošība — nekad neatstāj lietotāju bez atbildes."""
    try:
...
def nina_public_offer_answer(user_text=""):
    """V30.0: jebkuram lietotājam dzīva atbilde un piedāvājums."""
    ...
    return charm_smalltalk_answer(version="V30.0")



def nina_rough_message_answer():
    return charm_rough_answer(version="V30.0")
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """V30.0: Telegram foto apstrāde ar Vision Engine."""
    try:
        user_id = str(update.effective_user.id)
        ...
        # V30.0: enhance Vision answer
        answer = v30_enhance_vision_answer(user_id, answer, caption)
        try:
...
async def reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # V30.0 public reply wrapper
    try:
        user_text = update.message.text
        user_id = str(update.effective_user.id)
        lower = user_text.strip().lower()

        # V30.0: Relationship + Extended Profile + Smart Flow route.
        ...
        v30_fact = v30_detect_extended_profile_fact(user_text)
        if v30_fact.get("type") and v30_fact.get("value"):
            try:
                v30_save_profile_fact(user_id, v30_fact["type"], v30_fact["value"])
            except Exception as e:
                print("V30 profile save kļūda:", repr(e))
            await safe_reply_text(update, v30_profile_saved_answer(user_id, v30_fact["type"], v30_fact["value"]))
            return

        v30_answer = v30_followup_answer(user_id, user_text)
        if v30_answer:
            await safe_reply_text(update, v30_answer + v30_soft_premium_line(user_id))
            return


        # V30.0: User Profile DB route.
        if any(x in lower for x in ["ko tu par mani atceries", "ko atceries par mani", "mans profils", "profils"]):
            await safe_reply_text(update, v24_profile_recall_answer(user_id))
            return
        ...
        # V30.0: Smart Sales price/tariff route.
        if v20_is_price_question(user_text):
            await safe_reply_text(update, v20_smart_price_answer(user_id))
            return


        # V30.0: HARD priority capabilities route before old dialog/charm.
        ...
        # V30.0: Human Mode priority route. Tam jābūt pirms vecā dialog.py.
        if v18_should_use_human_mode(user_text):
            await safe_reply_text(update, v19_human_mode_answer_with_memory(user_id, user_text))
            return


        # V30.0: Living Conversation Core priority route.
        if v18_should_use_human_mode(user_text):
            await safe_reply_text(update, v19_human_mode_answer_with_memory(user_id, user_text))
            return


        # V30.0: Dialog smart route. Jautājumi nav atmiņas.
        dialog_kind = classify_dialog_message(user_text)
        if dialog_kind == "capabilities" or dialog_kind == "question":
            await safe_reply_text(update, build_capabilities_answer(version="V30.0"))
            return

        if dialog_kind == "rough_playful":
            await safe_reply_text(update, build_playful_rough_answer(version="V30.0"))
            return

        if dialog_kind == "smalltalk":
            await safe_reply_text(update, build_smalltalk_answer(user_text, version="V30.0"))
            return


        # V30.0: Always Reply public safety.
        if looks_like_rough_message(user_text):
            await safe_reply_text(update, nina_rough_message_answer())
            return


        # V30.0: Progress command.
        ...
        # V30.0: Reminder command.
        reminder_data = parse_reminder_request(user_text, DEFAULT_TIMEZONE)
        if reminder_data is not None:
            if not reminder_data.get("ok"):
                await update.message.reply_text(build_reminder_help_answer(version="V30.0"), disable_web_page_preview=True)
                return
        ...
        # V30.0: Mana diena top-priority route.
        ...
        # V30.0: Natural memory and daily goal capture — immediate replies.
        ...
        # V30.0: Natural conversation auto-handle.
        natural_kind = classify_natural_message(user_text)

        if natural_kind == "goal":
            goal_text = user_text.strip()
            ok = save_daily_goal(user_id, goal_text)
            answer = build_auto_goal_answer(goal_text, version="V30.0") if ok else "Neizdevās saglabāt mērķi. Pamēģini vēlreiz."
            await update.message.reply_text(answer, disable_web_page_preview=True)
            return
        ...
        if natural_kind == "memory":
            ...
            if saved:
                record_memory_topics(user_id, saved)
            answer = build_auto_memory_answer(saved or memory_text, version="V30.0") if saved else "Neizdevās saglabāt. Pamēģini vēlreiz."
            await update.message.reply_text(answer, disable_web_page_preview=True)
            return
    ...
    """
        ...
        # V30.0: Final catch-all so Nina never stays silent.
...
@app.route("/")
def home():
    return "Nina7727 V30.0 Premium Sales Text darbojas! DB: " + ("PostgreSQL" if USE_POSTGRES else "SQLite fallback")
...
if __name__ == "__main__":
    print("Nina7727 V30.0 Premium Sales Text darbojas...", "PostgreSQL" if USE_POSTGRES else "SQLite fallback")
