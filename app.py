...
def subscription_info(user_id=None):
    """V13.3: Premium pārdošanas teksts ar cilvēkam saprotamu vērtību."""
...
def premium_conversion_answer(user_id):
    """V13.3: labāks Free -> Premium pārdošanas teksts."""
    user = get_user(user_id)
    if user.get("premium"):
...
# =========================
# V13.3 NATURAL MEMORY + DAILY GOALS
# =========================

def save_daily_goal(user_id, goal_text):
    """V13.3: Saglabā šodienas mērķi, aizvietojot veco šodienas mērķi."""
...
def latest_daily_goals(user_id, limit=3):
    """V13.3: Atgriež šodienas aktīvo mērķi/mērķus."""
    today = datetime.now(ZoneInfo(DEFAULT_TIMEZONE)).strftime("%Y-%m-%d")
...
def latest_natural_memories(user_id, limit=3):
    """V13.3: Atgriež pēdējās svarīgās lietas, ko lietotājs palūdza atcerēties."""
    try:
        conn = get_db()
        c = conn.cursor()
        db_execute(
...
def nina_daily_habit_answer(user_id):
    """V13.3: Daily Assistant rāda šodienas mērķi un pēdējās svarīgās atmiņas."""
    user = get_user(str(user_id))
    ...
    memories = latest_natural_memories(str(user_id), limit=3)
    ...
    if memories:
        ...
        for row in memories:
            memory_lines.append(f"• {row[0]}")
        memories_text = "\n".join(memory_lines)
    else:
        memories_text = "🧠 Es vēl neatceros nevienu svarīgu lietu, ko esi man uzticējis."

    if reminders == 0:
        reminder_text = "⏰ Šobrīd tev nav aktīvu atgādinājumu."
    else:
        reminder_text = f"⏰ Tev ir {reminders} aktīvi atgādinājumi."
...
def nina_evening_answer(user_id):
    """V13.3: vakara kopsavilkuma sagatave."""
    return (
...
async def reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # V13.3: Natural memory and daily goal capture — immediate replies.
...
@app.route("/")
def home():
    return "Nina7727 V13.3 Premium Sales Text darbojas! DB: " + ("PostgreSQL" if USE_POSTGRES else "SQLite fallback")
...
if __name__ == "__main__":
    print("Nina7727 V13.3 Premium Sales Text darbojas...", "PostgreSQL" if USE_POSTGRES else "SQLite fallback")
