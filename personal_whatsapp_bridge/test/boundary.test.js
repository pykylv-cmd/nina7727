...
import {disconnectDetails,normalizeStoredKey,reconnectDelay,resolveWhatsAppVersion} from '../src/session_manager.js'
import {clearAuth} from '../src/nina_api.js'
...
test('fresh pairing clears every persisted auth record without exposing values',async()=>{
  ...
  globalThis.fetch=async(url,options)=>{requests.push({url,body:JSON.parse(options.body)});return {ok:true,json:async()=>requests.length===1?{records:{creds:{registered:false},'key:session:1':{private:'hidden'}}}:{ok:true}}}
